...
5. Validate the complete process-exit shape and its producer context centrally before any consumer may narrow recovery. Exactly one abnormal termination fact is required: an OS exit code in the range 1–255 or a Node-recognized signal name. The enclosing diagnostic must be the `workflow-engine-defect` / `harness-failure` / `loop-dispatch` tuple. Authoritative observation failure or durable start-linkage rejection retains an already-observed abnormal termination because neither is contrary completion evidence.
