...
- [ ] 1.1 Extend `DispatchResumeDeps` in `core/scripts/stages/planning.ts` with the recovery
...
- [ ] 1.2 In `core/scripts/pipeline-run.ts`, have the `implementing` case in `dispatch()` pass
...
- [ ] 2.1 Add a liveness gate at the top of `dispatchResume` (after the dry-run short-circuit):
...
- [ ] 2.2 Keep the existing resume path: no live owner AND worktree with commits ahead of
  `cfg.base_branch` → run gate → push → PR → `review-1` (unchanged).
- [ ] 2.3 Replace the old no-commits `waiting` return with the recovery branch: no live owner
  AND no commits →
...
- [ ] 2.4 Ensure the liveness check runs BEFORE the commits-ahead check (ordering matters for
  the resume-race guarantee).

## 3. Unit tests (`core/test/implementing-crash-recovery.test.ts` or extend planning tests)

- [ ] 3.1 Crash-stranded (marker absent/dead, no worktree commits): fake `isLivePlanningActive`
...
- [ ] 3.2 Live owner (marker PID alive): fake `isLivePlanningActive` → true. Assert a
...
- [ ] 3.3 Resume preserved (no live owner, commits present): fake `isLivePlanningActive` →
...
- [ ] 3.4 Ordering: with a live marker AND a commit-bearing worktree, assert the outcome is
  `waiting` (liveness short-circuits before the commits check) — proves the gate ordering.
- [ ] 3.5 Prove the tests bite: revert task 2.3 (restore the old no-commits `waiting` return),
  run the suite, confirm 3.1 (and 3.4 as applicable) fail, then restore.

## 4. Mirror + CI

- [ ] 4.1 `node scripts/build.mjs` — regenerate the `plugin/` mirror.
