...
Pins the Factory Reliability Gate (FRG) `clean-docs` pack instance for run `pack-1401-pipeline-ship-1.40.1` to a run-scoped JSON fixture whose `release_version` is `1.40.1`, plus a unit test that fails if that value changes.
