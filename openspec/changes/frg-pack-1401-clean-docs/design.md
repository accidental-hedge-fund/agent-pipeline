...
- Issue #1464 is a synthetic FRG `clean-docs` item. The pack run id is
  `pack-1401-pipeline-ship-1.40.1`. The release is `1.40.1`.
- `core/test/fixtures/frg/` does not exist on the issue base. The
  implementation creates that tree.
- Prior pack instances (not on this base, because FRG closes without merge)
  already used the disk-read pattern: commit `49370b8f`
  (`core/test/frg-pack-1400-clean-docs.test.ts`) and commit `7d801f4c`
  (`core/test/frg-pack-13916-clean-docs.test.ts`). Those tests resolve
  `./fixtures/frg/<pack_run_id>/clean-docs.json` from `import.meta.url`,
  `readFileSync`, `JSON.parse`, and `assert.equal`.
- On this base, `core/test/` already uses `node:test`, `node:fs`,
  `fileURLToPath(import.meta.url)`, and `JSON.parse` (for example
  `core/test/version.test.ts` and `core/test/candidate-engine-readiness.test.ts`).
- `scripts/build.mjs` regenerates host SKILLs from renderer sources
  (`core/scripts/host-skill.ts`, `core/scripts/operation-surface.ts`). A
  `core/test/`-only change does not require that regeneration. The generator
  does not write `plugin/` (#1050).
...
Copy the pack-1400 disk-read `node:test` pattern. Use stdlib `node:fs`,
`JSON.parse`, and `node:assert/strict`. Do not invent a fixture loader,
schema, helper module, or production import.
...
- Production engine, CLI, stage, config, or host edits.
- Recreating `plugin/`.
...
**Choice:** Add `frg-pack-1401-clean-docs` as a new capability. Do not
...
This issue's contract is a synthetic pack-run fixture that names `1.40.1`.
...
the string `1.40.1`. The unit test asserts that key only.

**Why:** The sibling FRG template `clean-openspec` names the same field.
The issue requires the fixture to name release `1.40.1`, not a full pack
manifest. Pack-1400 used this exact object with `1.40.0`.
...
**Choice:** Add `core/test/frg-pack-1401-clean-docs.test.ts`. The test
resolves
...
`release_version === "1.40.1"`. It does not import `core/scripts/`.
...
### 4. No host SKILL regeneration

**Choice:** Implementation edits `core/test/` only. Do not run
`node scripts/build.mjs` unless a later review forces a `core/scripts/`
edit (that edit is out of scope). Do not recreate `plugin/`.

**Why:** `scripts/build.mjs` reads renderer sources under `core/scripts/`,
not `core/test/`. Whole-tree `plugin/` is retired (#1050).
...
3. Pre-merge archives `frg-pack-1401-clean-docs` into living specs. No
