...
- [ ] That run exits with code `2` (distinct from `0` success and `1` stop/hold) and prints a
...
- [ ] The terminal JSON carries additive `dispatched`, `excluded`, `excluded_item_ids`,
      `exclusion_reason`, and `completion` keys; `schema_version` and every pre-existing key's
      name and type are unchanged.
- [ ] A fully-completed run with no exclusions still reports `all_done: true`,
      `completion: "all_done"`, `excluded: 0`, `exclusion_reason: null`, and exits `0`.
- [ ] A mixed run reports `all_done: false`, `completion: "partial_excluded"`, non-zero
      `dispatched` and `excluded`, exits `0`, and still prints the excluded-count line.
- [ ] A stop or outstanding hold keeps its current reporting and exit code `1`, with
      `completion: null` and unchanged `stop` / `hold_outstanding` / `held_item_ids`.
- [ ] The dominant exclusion reason is deterministic (most frequent reason; ties broken by a
      stable ordering).
- [ ] Held items are not counted as excluded and keep their own disclosure.
- [ ] No change to precondition/exclusion semantics: still non-fatal, re-evaluated each cycle,
      no recovery budget consumed, no run stop, mid-run triage still admits the item.
- [ ] Regression test that bites: an all-excluded work list yields `all_done: false` /
...
1. [ ] In `core/scripts/loop/supervisor.ts`, narrow the resolving-cycle report: keep the
       ...
       run resolves.
2. [ ] Extend `SupervisorCycleResult` and `DriveSupervisorResult` with the accounting fields
       ...
       run's accounting.
3. [ ] Add the deterministic dominant-reason helper (most frequent exclusion reason; ties broken
...
4. [ ] Derive the `completion` classifier (`all_done` | `partial_excluded` | `none_dispatchable`,
       ...
       `all_done`.
5. [ ] In `core/scripts/pipeline.ts` `runLoopCommand`, emit the additive JSON keys
...
6. [ ] Tests (co-located `*.test.ts`, dependency-seam fakes, no real network/git/subprocess):
       - [ ] Regression: all-excluded work list → `all_done: false`,
             `completion: "none_dispatchable"`, `dispatched: 0`, `excluded: N`, exit `2`, CLI
             line printed. Verify it fails without the fix.
       - [ ] Fully-completed run → `all_done: true`, `completion: "all_done"`, `excluded: 0`,
             `exclusion_reason: null`, exit `0`, no excluded line.
       - [ ] Mixed run → `partial_excluded`, non-zero counts, exit `0`, excluded line printed.
       - [ ] Stop and outstanding-hold runs → `completion: null`, exit `1`, existing
             disclosures unchanged.
       - [ ] Dominant-reason helper: most frequent reason wins; a tie resolves deterministically.
       - [ ] Held item alongside excluded items is not counted in `excluded` /
             `excluded_item_ids` and is still named by the held-item disclosure.
       - [ ] Resumed run counts pre-resume terminal-successful items in `dispatched`.
       - [ ] Semantics preserved: an excluded item triaged to the required stage mid-run is
             admitted on a later cycle; an all-excluded run resolves without a watchdog stop.
7. [ ] Update the existing `core/test/loop-command.test.ts` `all_done: true` assertion so it
       covers a genuinely all-done run (no excluded items) rather than the conflated case.
8. [ ] Document the terminal-summary fields and the `2` exit code wherever the loop command's
...
9. [ ] Run `node scripts/build.mjs` to regenerate the `plugin/` mirror; commit it in the same
       change.
10. [ ] Run `npm run ci` from the repo root; treat red as not-done.
