...
Issue #1255 is a factory-gate `clean-docs` pack instance for pack run
`pack-13913-release-v1.39.13-frg-20260827-1530` (release `1.39.13`). The Factory
Reliability Gate (FRG) needs one clean Pipeline path that lands a small
documentation fixture and a unit test. Production pipeline behavior does not
change.
...
- The fixture names `release_version` as `1.39.13`.
...
- [ ] File `core/test/fixtures/frg/pack-13913-release-v1.39.13-frg-20260827-1530/clean-docs.json`
      exists and parses as JSON.
- [ ] That fixture's `release_version` field equals the string `1.39.13`.
- [ ] A unit test in `core/test/` reads only that run-scoped path (no other
      pack-run fixture path).
- [ ] That test fails when the fixture's `release_version` is not `1.39.13`.
...
- `frg-pack-13913-clean-docs`: Run-scoped clean-docs fixture and unit test for
  pack run `pack-13913-release-v1.39.13-frg-20260827-1530` (release `1.39.13`).
  The fixture pins `release_version`. The test fails if that value changes.
...
- **Tests only:** `core/test/fixtures/frg/pack-13913-release-v1.39.13-frg-20260827-1530/clean-docs.json`
