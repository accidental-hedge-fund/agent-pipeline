...
- **THEN** the compact one-pager SHALL name the retained-id advance command and the shared material filter
- **AND** it SHALL NOT recommend the broad stdout alternation as the primary filter
- **AND** for `pipeline single` / loop it SHALL also name the retained-id loop command and the linked-advance command after linkage
