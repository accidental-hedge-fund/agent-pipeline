...
- FROM: ### Requirement: Issue-scoped Monitor grep filter
- TO: ### Requirement: Run-store event follow SHALL be the primary host monitoring contract
- FROM: ### Requirement: Rationale for tight filter documented
- TO: ### Requirement: Durable docs SHALL explain material filtering and notification-spam suppression
- FROM: ### Requirement: No real signal lost by tight filter
- TO: ### Requirement: Run-store event follow SHALL preserve terminal and diagnostic evidence
- FROM: ### Requirement: Consistent filter across all host variants
- TO: ### Requirement: Generated hosts SHALL share one compact event-follow contract
- FROM: ### Requirement: Host skill guidance SHALL document the events.jsonl material filter for progress notify
- TO: ### Requirement: Shared renderer and durable docs SHALL document the events.jsonl material filter for progress notify
- FROM: ### Requirement: Material filter guidance SHALL be consistent across host variants
- TO: ### Requirement: Shared filter and durable docs SHALL own one material-event inventory

## MODIFIED Requirements

### Requirement: Run-store event follow SHALL be the primary host monitoring contract

`core/scripts/host-skill.ts` and every generated host SKILL SHALL use structured run-store event follow as the primary monitoring contract. The default numeric `pipeline <N>` / `pipeline single` path SHALL retain the handoff `loop_run_id` and follow `pipeline loop logs <loop-run-id> --events --follow`; after `loop_item_advance_linked`, it SHALL retain that event's `pipeline_run_id` value as `advance_run_id` and also follow `pipeline logs <advance-run-id> --events --follow`. The compact contract SHALL direct human-visible progress from both applicable streams through the shared material filter or an equivalent `--material` surface. It SHALL NOT recommend the broad stdout alternation `"^\[pipeline\]|^\[exit code|FAILED|timed out|blocked label|approved|needs-attention|→ "` as the primary filter. Durable docs MAY retain the legacy issue-scoped stdout grep `^\[pipeline\] #<N>: ` as a diagnostic for legacy logs, but each generated one-pager SHALL carry only the structured retained-id follow guidance and a durable-doc pointer.

#### Scenario: Tight filter used in Monitor command

- **WHEN** an operator arms a host follow for a default numeric/single drive
- **THEN** the compact one-pager SHALL name the retained-id loop command, the linked-advance command after linkage, and the shared material filter
