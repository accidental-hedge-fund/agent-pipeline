...
- Issue #1256 is a synthetic FRG `clean-openspec` item. The pack run id is
  `pack-13913-release-v1.39.13-frg-20260827-1530`. The release is `1.39.13`.
- `core/test/fixtures/frg/` does not exist on the issue base. The
  implementation creates that tree.
- Existing tests load fixtures from `core/test/fixtures/` via
  `path.dirname(fileURLToPath(import.meta.url))` (see
  `core/test/outer-host-lifecycle.test.ts`).
- `scripts/build.mjs` mirrors `core/scripts`, `core/profiles`, and
  `core/package*.json` only. A `core/test/`-only change does not require a
  `plugin/` regeneration.
...
**Choice:** Add `frg-13913-clean-openspec` as a new capability. Do not modify
`openspec/specs/factory-reliability-gate/spec.md`.

**Why:** The living FRG spec describes the gate driver and pack inventory.
This issue's contract is a synthetic pack-run fixture that names
`1.39.13`. Mixing that into FRG driver law would couple a one-run artifact
to release-precondition rules.

**Alternatives considered:**

- Delta on `factory-reliability-gate` → rejected. Wrong layer.
- `skip_specs: true` → rejected. The issue requires one OpenSpec
  requirement that the fixture names `1.39.13`.
...
```json
{
  "release_version": "1.39.13"
}
```

Optional extra keys are allowed. The required key is `release_version` as
the string `1.39.13`. The unit test asserts that key only.

**Why:** The sibling FRG template `clean-docs` names the same field. The
issue requires the fixture to name release `1.39.13`, not a full pack
manifest.
...
**Choice:** Add `core/test/frg-13913-clean-openspec.test.ts` (or an equivalent
co-located `core/test/*.test.ts` name). The test joins
...
`release_version === "1.39.13"`. It does not import `core/scripts/`.
...
3. Pre-merge archives `frg-13913-clean-openspec` into living specs. No
