...
- **Reuse first:** alias public mutating numeric drive to existing `runSingleIssueCommand`. Keep existing `runAdvance`, `pipeline/loop-execution@1`, `dispatchItemChildArgs`, `loop_item_advance_linked`, and `formatLoopRunHandoff`. Nested dispatch uses a dedicated child executor rather than a public env-marker admission. Do not add a second one-item scheduler, a public supervisor verb, or host-owned retry.
