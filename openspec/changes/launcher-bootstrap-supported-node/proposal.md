...
- **Re-exec onto Node ≥ 24 for every TypeScript-loading route** (`status`, `train`, `path`, bare invoke, …). Resolution SHALL reuse `scripts/ensure-engines-node.mjs` (`resolveEnginesNode` / `envPreferringNode` / `AGENT_PIPELINE_NODE`). The child's `PATH` SHALL prefer that binary's directory. Argv SHALL be preserved.
...
- [ ] A TypeScript-loading command (`status`, `train`, `path`, bare invoke) invoked with a still-supported Node major below the engine floor (today: 22) re-execs onto a resolved Node ≥ 24 binary, preserves argv, and puts that binary's directory first on the child's `PATH`. Resolution uses `scripts/ensure-engines-node.mjs` (`resolveEnginesNode` / `envPreferringNode` / `AGENT_PIPELINE_NODE`), not a second walker.
