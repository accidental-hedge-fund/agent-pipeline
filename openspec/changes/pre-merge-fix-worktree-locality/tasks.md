...
- [ ] 1.1 Re-trace the pre-merge bounded auto-fix path and confirm the harness cwd
...
- [ ] 1.2 Confirm the lyric-utils run `648/2026-07-23` clean-worktree signature classification
...
- [ ] 1.3 Write the determination to issue #547 so its salvage-extension scope is confirmed
      sufficient or amended.

## 2. Pin worktree locality for the full-fix executor seam

- [ ] 2.1 Inspect `invokeStageExecutor` (`fix.ts:572`) and `core/scripts/executors.ts`: verify
...
- [ ] 2.2 If a divergence is possible, ensure the executor runs in the issue worktree (or the
...
- [ ] 3.1 In `performPreMergeAutoFix`, replace the silent clean/no-commit escalation with a
...
- [ ] 3.2 Ensure the equivalent full-fix path surfaces the same disclosure when the harness ran
      but the inspected worktree is clean with no commit.

## 4. Regression tests (deps seams only — no real subprocess/git/network)

- [ ] 4.1 Add a worktree-locality test asserting the cwd handed to the fix harness equals the
      path handed to the salvage/status inspection; prove it bites by routing the harness to a
      different checkout.
- [ ] 4.2 Add a disclosure test asserting the clean/no-commit pre-merge path escalates with a
      diagnostic containing the inspected worktree path; prove it bites when the disclosure is
      removed.
- [ ] 4.3 Confirm the existing `pre-merge-autofix` `#547` salvage and rollback tests still pass
      unchanged.

## 5. Mirror + gate

- [ ] 5.1 Regenerate the mirror: `node scripts/build.mjs`; confirm `node scripts/build.mjs
      --check` is clean.
