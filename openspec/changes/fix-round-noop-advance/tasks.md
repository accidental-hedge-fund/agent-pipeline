...
- [ ] 1.1 In `advanceFix` (fix.ts), after building `trustedForAck`, derive live
...
- [ ] 1.2 When the effective blocking set is empty, skip the harness invocation and
...
- [ ] 1.3 When the effective set is partial, invoke the harness with only the
...
- [ ] 2.1 `fix.md`: add the sanctioned outcome — when the harness makes no change
...
- [ ] 2.2 `prompt-loader.test.ts`: add a drift assertion that bites when the
...
- [ ] 3.1 In `advanceFix`, on the `headBefore === headAfter` + salvage-empty path,
...
- [ ] 3.2 When every invoked blocking finding is covered by a valid declaration,
...
- [ ] 3.3 Extract the declaration parser/validator as an exported pure function
...
- [ ] 4.1 On a does-not-reproduce advance, post an audited disposition comment
...
- [ ] 4.2 At fix/review entry, consult recorded non-reproducing dispositions
...
- [ ] 5.1 The "block when HEAD equals reviewed SHA" path yields to two carve-outs:
...
- [ ] 6.1 All triggering blockers overridden → harness NOT invoked; advances to the
      correct next stage; no `setBlocked`.
- [ ] 6.2 Partial overrides → harness invoked; prompt scope excludes the overridden
      findings; includes the remaining ones.
- [ ] 6.3 All invoked blockers declared non-reproducing (valid key + matching SHA)
      → advances; no `no-commits` block; disposition sentinel posted.
- [ ] 6.4 A no-op round with an invoked blocker neither committed, overridden, nor
      validly declared → still blocks with `blockerKind: "no-commits"` (fail closed).
- [ ] 6.5 A declaration with a key outside the invoked set, or a SHA ≠ current HEAD,
      is ignored (does not force an advance).
- [ ] 6.6 SHA-anchored consultation: re-entry at the same reviewed SHA suppresses the
      finding; re-entry after a SHA change re-opens it.
- [ ] 6.7 Genuine reproducing finding → normal fix-and-commit flow unchanged.

## 7. Mirror + CI

- [ ] 7.1 `node scripts/build.mjs`; `npm run ci` green.
