...
- [x] 1.1 Add an explicit sandbox-mode input to harness invocation shaping: a new optional
...
- [x] 1.2 Add the manifest field (`ExperimentManifest` in `core/scripts/evals/types.ts`) with a
...
- [x] 1.3 Fold the resolved mode into `effectiveConfig` in `runCell` (so it enters `config_hash`) and
  onto `CellRecord` in `run.ts`; extend `CellRecord` in `types.ts`.
- [x] 1.4 Pass the resolved mode from `runCell` through `realInvokeHarness` → `invoke()`; the eval
  path must never read `PIPELINE_CODEX_NO_SANDBOX`.
- [x] 1.5 Tests: `harness.test.ts` — explicit mode selects each shape and differs only in the
...
- [x] 2.1 Add a single-sourced contract asset under `core/scripts/evals/` (e.g.
...
- [x] 2.2 Write the contract text covering the four required clauses: frozen-task-only scope; the
...
- [x] 2.3 Add install/restore helpers in `core/scripts/evals/executor.ts` with a dependency seam
  (read/write/remove), capturing prior content in memory before writing.
- [x] 2.4 Wire install before the first harness invocation in `runCell`; return `infra_error` without
  invoking the harness if installation fails.
- [x] 2.5 Wire restore before `runChecks` and before `getChangedPaths`, and again (idempotently) in
...
- [x] 2.6 Exclude the contract paths and the denial log from `detail.changed_paths` in
  `defaultGetChangedPaths`'s caller.
- [x] 2.7 Tests: contract-drift test fails when any required clause is removed;
...
- [x] 3.1 Add a cell-scoped deny-shim builder in `core/scripts/evals/executor.ts` (cell-scoped
...
- [x] 3.2 Each interceptor prints a named denial reason to stderr, exits non-zero, and appends one
  JSON line (`{ command, argv, category, at }`) to the cell-scoped denial log.
- [x] 3.3 Prepend the shim directory to `PATH` in `isolatedGhEnv` (or a sibling that composes with
  it), keeping the change cell-scoped and leaving non-eval invocations untouched.
- [x] 3.4 Read and parse the denial log after the harness returns; surface entries alongside the
  existing `ghRefusals`.
- [x] 3.5 Tests: `evals-executor.test.ts` with injected fakes — denied categories produce a denial
  record and a non-zero result; a permitted git operation is passed through; the boundary env is
  scoped to the cell.

## 4. Durable boundary evidence

- [x] 4.1 Add a boundary-evidence field to `CellRecord` (`core/scripts/evals/types.ts`) carrying both
  process denials and `gh`-surface refusals, plus a collection-failure reason field mirroring
  `trajectory_artifact_error`.
- [x] 4.2 Persist it in `run.ts` — `runCell` already returns `ghRefusals` and today's `run.ts`
  **drops them**; carry both channels onto the record before `appendCellRecord`.
- [x] 4.3 Confirm and pin that a boundary denial does not change `result_class` and that no grader in
  `core/scripts/evals/grading/` reads the new field.
- [x] 4.4 Record boundary events on the trajectory `actions` list for diagnosis (kept out of grading
  input, consistent with #536's hidden-material containment).
- [x] 4.5 Tests: `evals-run.test.ts` — a cell whose injected harness triggers a denial and a `gh`
...
- [x] 5.1 Document the boundary and the declared sandbox mode in the evals documentation surface
  (manifest field, what is denied, where denials are recorded, and the known `PATH`-shim bypass
  limitation).
- [x] 5.2 `node scripts/build.mjs` and commit the regenerated `plugin/` mirror.
- [x] 5.3 `npm run ci` green from the repo root.

## 6. Validation against the reported failure

- [x] 6.1 Re-check each acceptance criterion in `proposal.md` against the implemented behavior.
- [x] 6.2 Confirm the two observed #607 modes are now covered: an externally sandboxed runner is
