...
- [ ] 1.1 Add a pure selection helper (e.g. `selectSupersededOpenPrs`) that, given open
...
- [ ] 1.2 Confirm or extend the open-candidate shape / list path so base ref is available
      ...
      title/body search).
- [ ] 1.3 Add config support for `supersede_mode`: `close` (default when absent) and
      `comment-only`, matching existing `config.ts` / pipeline.yml schema patterns.

## 2. Dispose path and implement hook

- [ ] 2.1 Implement async dispose helper with injectable deps (`listOpenPrsForIssue` or
...
- [ ] 2.2 Fail-soft per PR and on list failure: log diagnostics; do not fail managed PR
      ensure solely because supersede failed.
- [ ] 2.3 Hook dispose into `resumeFromImplementing` after create-or-reuse yields managed
      PR M for head H (both new and reused paths); do not change exact-head reuse or
      create-once race re-lookup.
- [ ] 2.4 Ensure comment/close never targets M itself, never merges, and never force-pushes
      or deletes superseded branches.

## 3. Unit tests (injected seams only)

- [ ] 3.1 Fixture: two open associated PRs for N on different heads, same base → after
...
- [ ] 3.2 Prove bite: same fixture without dispose (or with dispose no-op) leaves both
      open / does not call close — assertion fails if disposal is skipped in production path.
- [ ] 3.3 Comment-only mode: stale PR receives `pipeline-superseded` comment naming
      superseding PR and remains open.
- [ ] 3.4 Unrelated PR (no dual-strategy association) and different-base associated PR are
      not closed or supersede-commented.
- [ ] 3.5 Reuse path (existing managed PR on H) still disposes other-head associated PRs.
- [ ] 3.6 Fail-soft: close failure on one candidate does not fail the ensure-PR outcome;
      other candidates still attempted.

## 4. Mirror, validate, gate

- [ ] 4.1 After any `core/` edit, run `node scripts/build.mjs` and include regenerated
      `plugin/` in the same commit.
- [ ] 4.2 Run `openspec validate close-superseded-issue-prs` (and keep deltas archive-ready).
- [ ] 4.3 Run `npm run ci` from repo root; fix failures before done.
