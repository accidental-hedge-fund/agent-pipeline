...
- [ ] 3.1 Confirm `git diff` for this implementation has no edits under `core/scripts/`, `hosts/`, or pack templates. Only the fixture, the new test, regenerated `plugin/` (if the test is mirrored), and OpenSpec archive (pre-merge) are in scope
- [ ] 3.2 After any `core/` edit, run `node scripts/build.mjs` from the repo root and include regenerated `plugin/` in the same change. Verify `node scripts/build.mjs --check` is clean
