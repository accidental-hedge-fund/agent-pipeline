...
See `proposal.md` for why. Issue #1208 is a synthetic Factory Reliability
Gate (FRG) pack item (`template_id=clean-openspec`,
`pack_run_id=pack-13910-tugboat-ship-1.39.10`). It is not an engine-dogfood
...
path. The next identical pack run files a new instance from
`core/scripts/frg-packs/factory-gate-v1/templates/clean-openspec.md`. It
does not need a new mole issue.
...
Sibling pack item #1207 (`clean-docs`) uses the same
`pack-13910-tugboat-ship-1.39.10` directory on a separate branch. This
change adds `clean-openspec.json` beside it. It does not replace that
sibling file.
...
- One co-located unit test that asserts `release_version === "1.39.10"`.
- Keep this the only active OpenSpec change for #1208.
...
- Edits to a sibling `clean-docs.json` in the same pack-run directory.

## Decisions

### 1. JSON field is `release_version`

**Choice:** The fixture uses `release_version` with value `1.39.10`.
...
**Choice:** Write
`core/test/fixtures/frg/pack-13910-tugboat-ship-1.39.10/clean-openspec.json`.
...
**Choice:** Add `core/test/frg-pack-13910-clean-openspec.test.ts`. It reads
...
contract tests. Sibling #1207 uses the same read-and-assert shape.
...
expand the OpenSpec contract past the pack item. Sibling #1207 ships
`{ "release_version": "1.39.10" }` only.
...
- [Sibling #1207 already owns the pack-run directory] → Add only
  `clean-openspec.json`. Do not delete or rewrite `clean-docs.json`.

## Migration Plan

No production migration. Pre-merge archives
`frg-pack-13910-clean-openspec` into
`openspec/specs/frg-pack-13910-clean-openspec/spec.md`. Rollback is revert
