...
Issue #1208 is the Factory Reliability Gate (FRG) `clean-openspec` pack item
for `pack-13910-tugboat-ship-1.39.10`. The pack must exercise one clean
...
- The fixture SHALL name release `1.39.10`.
- Add one unit test that reads only that path and verifies the release
  value.
- Author this OpenSpec change for issue #1208 only. Pre-merge archives it.
...
- [ ] The file `core/test/fixtures/frg/pack-13910-tugboat-ship-1.39.10/clean-openspec.json` exists and is valid JSON.
- [ ] That fixture sets `release_version` to `1.39.10`.
- [ ] A co-located unit test under `core/test/` reads only that run-scoped path and fails if the named release is not `1.39.10`.
...
- [ ] The only active OpenSpec change for this work is `frg-pack-13910-clean-openspec`.
...
- [ ] The Pipeline for #1208 reaches `pipeline:ready-to-deploy`. The FRG closes the pull request and issue without merge after it records the run.

## Capabilities

### New Capabilities

- `frg-pack-13910-clean-openspec`: Run-scoped FRG `clean-openspec` fixture
  for `pack-13910-tugboat-ship-1.39.10` must name release `1.39.10`. A unit
...
- **Tests:** new JSON fixture under
  `core/test/fixtures/frg/pack-13910-tugboat-ship-1.39.10/` and one
  co-located unit test.
- **OpenSpec:** one new living spec after archive
  (`openspec/specs/frg-pack-13910-clean-openspec/spec.md`).
