...
- Install a native OpenCode `/pipeline` command in the documented OpenCode `commands/` location that routes user arguments to the installed pipeline launcher, including `--version` / `-V`.
...
- [ ] After that install, the OpenCode `/pipeline --version` (and `-V`) path yields the same version string as the installed skill launcher’s `pipeline --version` / `-V` (matching `core/package.json` at the install root), and does not surface generic pipeline instructional skill text as the response for that invocation.
...
- `opencode-pipeline-command`: Native OpenCode `/pipeline` command surface that routes arguments to the installed launcher safely and short-circuits `--version`/`-V` to launcher version output rather than instructional skill text.
