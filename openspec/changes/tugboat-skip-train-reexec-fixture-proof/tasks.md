...
- [x] 3.2 Do not change `skip_train_has_proof` or `ensure_train_complete_artifact`. Verify existing #1182 empty-milestone and RUN_DIR-evidence tests still pass
...
- [x] 4.2 Run `openspec validate tugboat-skip-train-reexec-fixture-proof` and `npm run ci` from the repo root. Verify both are green. Verify the four named tests pass with parent skip-train env injected. Do not add `--skip-frg` as the default ship path. Do not re-implement #1181–#1183
