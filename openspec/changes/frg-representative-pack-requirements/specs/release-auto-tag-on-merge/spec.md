...
required for release-eligible pass, and other release-eligibility failures defined by the
`factory-reliability-gate` capability (including representative pack composition when encoded on
the artifact). On validation failure the workflow SHALL exit non-zero and SHALL NOT create or
push any tag. On validation success the workflow MAY proceed to resolve notes and push the
annotated tag as already specified. Non-release pushes remain successful no-ops and SHALL NOT
require FRG evidence. The FRG check SHALL NOT merge pull requests, enable auto-merge, or
substitute for human ownership of the release merge itself.
