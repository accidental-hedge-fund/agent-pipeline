...
- [ ] 2.1 Define the run-scoped ledger entry type (item pair, `parallelized`/`serialized` disposition,
...
- [ ] 2.2 Implement pure accumulation of the scheduler's per-pass planning records into the run-scoped
...
- [ ] 2.3 Wire the ledger append at the scheduler call site in `core/scripts/loop/supervisor.ts` so
      every pass's decisions are recorded, without altering the serialized-by-default selection.
- [ ] 2.4 Add a drift-guard test asserting every ledger reason is a member of the scheduler's closed
...
- [ ] 3.1 Add a composition test (`core/test/`) driving a mixed frontier — a proven-disjoint pair, an
...
- [ ] 3.2 Assert **no unproven pair is ever concurrently `in_progress`**: only the proven-disjoint pair
...
- [ ] 3.3 Assert the **run-scoped ledger** contains one entry per evaluated pair, each with a
      disposition and exactly one structured reason, reconstructable from durable state.
- [ ] 3.4 Assert **merge-class operations stay globally serialized for the whole run** — no start while a
      merge barrier is set, no two merge-class operations overlapping — and the run stops at
      `pipeline:ready-to-deploy`.
- [ ] 3.5 Assert **changed-file overlap discovered mid-run parks** the affected items and records a
      durable replan request naming the file, with no external mutation and no race into concurrent
      merge preparation.
- [ ] 3.6 Assert **full evidence reconstruction**: every action, conflict detection, and scheduling
...
- [ ] 4.1 Confirm the #531 hermetic composition simulation covers disjoint concurrency, serialized
      conflict, changed-file-overlap park/replan, and serialized merge-class integration, and that its
      evidence bundle references the run-scoped ledger.
- [ ] 4.2 Confirm the captured live-pilot evidence bundle (linked from the pilot issue) demonstrates the
      composed guarantee end-to-end.

## 5. Gate

- [ ] 5.1 Regenerate the mirror: `node scripts/build.mjs` and commit the updated `plugin/`.
- [ ] 5.2 `npm run ci` green from repo root (includes `openspec validate --all`); every new test proven
      to bite.
