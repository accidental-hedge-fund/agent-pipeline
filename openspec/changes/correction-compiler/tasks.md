...
- [ ] 1.1 Add `ClusterCategory` member `"correction"` in `core/scripts/improve.ts` and extend
      ...
      dedup).
- [ ] 1.2 Add `clusterCorrections(event, runId, clusters, ...)` that reads `correction_event`
      ...
      and excerpt.
- [ ] 1.3 Ensure category isolation: correction clusters never merge with `papercut`,
...
- [ ] 2.1 Count occurrences as distinct `correction_id`s within a `correction_key` cluster; also
      compute distinct run count and distinct item (issue/PR) count for the report.
- [ ] 2.2 Set the `correction` category `--min-occurrences` default to 2 (singletons visible in
      report/`--json`, never filed at default); keep other categories' default unchanged.

## 3. Control-level proposal + graduation ladder

- [ ] 3.1 Add a `proposeControlLevel(cluster)` helper that returns one of `instruction`,
...
- [ ] 3.2 Enforce the graduation ladder `documented rule -> skill/rubric -> eval ->
...
- [ ] 3.3 Render the proposal block into the issue body: the named control level (or
...
- [ ] 4.1 Keep cluster identity, dedup, and qualification a pure function of bounded fields;
...
- [ ] 5.1 Thread `correction` through the human-readable report and `--json` output, emitting the
...
- [ ] 5.2 `--apply` for correction clusters reuses the existing open-issue `[pipeline-improve]`
...
- [ ] 6.1 Add an opt-in `corrections` config block in `config.ts` mirroring the `papercuts`
...
- [ ] 6.2 Add a correction auto-file entry that reuses the papercut cluster→issue create
...
- [ ] 6.3 Document the single-host concurrency scope (#459) on the correction auto-file path in
      runtime/docs; do not claim cross-host global deduplication for it.

## 7. Tests + mirror + CI

- [ ] 7.1 Fixtures + tests: identical-key recurrence, semantically-different keys (no merge),
...
- [ ] 7.2 Category-isolation test (correction vs papercut/flaky-gate/token-waste with coinciding
      signals) and control-level/graduation-ladder assertions; prove each test bites without the
      change.
- [ ] 7.3 `node scripts/build.mjs` to regenerate the `plugin/` mirror; run `npm run ci` from
