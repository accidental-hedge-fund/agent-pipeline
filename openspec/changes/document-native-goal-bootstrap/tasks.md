...
- [ ] 1.1 In `hosts/claude/SKILL.md`, add a bootstrap subsection near the
      `/pipeline:loop` documentation showing the operator running `/goal` and
      then `/pipeline:loop …` for a durable run, in that order.
- [ ] 1.2 State the non-claims: the skill does not detect host `/goal` state,
      does not itself invoke/re-enter `/goal`, and does not control the native
      session lifecycle.
- [ ] 1.3 State that native completion is a host/user action taken after the
...
- [ ] 2.1 In `hosts/codex/SKILL.md`, add the symmetric bootstrap subsection
      showing `/goal` then `$pipeline:loop …`, in that order.
- [ ] 2.2 Mirror the same non-claim and host-owned-completion statements, noting
...
- [ ] 3.1 Add a co-located test under `core/test/` that reads both authored host
...
- [ ] 3.2 Add the symmetry assertion: neither host carries the other's loop token
      in its bootstrap step, and both carry the same non-claim set.
- [ ] 3.3 The test performs no network, git, or subprocess call.
- [ ] 3.4 Prove the test bites: temporarily remove the bootstrap block and
      confirm the test fails, then restore.

## 4. Regenerate mirror and gate

- [ ] 4.1 Run `node scripts/build.mjs` to regenerate `plugin/` and commit the
      mirror in the same change.
- [ ] 4.2 Run `npm run ci` from the repo root and confirm it is green
      (`ci:core`, `build.mjs --check`, install-smoke, `openspec validate --all`).

## 5. Validate the change

- [ ] 5.1 Run `openspec validate document-native-goal-bootstrap` and fix any
