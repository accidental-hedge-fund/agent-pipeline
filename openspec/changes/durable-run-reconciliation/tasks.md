...
- [ ] 1.1 Add `LoopExternalIdentity` to `core/scripts/loop/types.ts` (`issue_number`, `issue_open`,
...
- [ ] 1.2 Add the closed `LoopDriftClass` enum (`ledger-behind`, `ledger-ahead`, `external-absent`,
      `identity-mismatch`, `checks-regressed`) plus an `isLoopDriftClass` guard.
- [ ] 1.3 Add the closed `LoopNextAction` enum (`advance`, `await-checks`, `repair-forward`,
      `clear-merge-barrier`, `hold-for-human`, `noop`) plus an `isLoopNextAction` guard.
- [ ] 1.4 Retype `LoopReconciliation`: `observed` becomes `Record<string, LoopExternalIdentity>` and
...
- [ ] 2.1 Add a `ReconcileObserveDeps` seam (engine-owned) wrapping the existing typed `gh` wrappers
...
- [ ] 2.2 Implement `observeExternalIdentity(deps, repo, item)` returning a `LoopExternalIdentity`
      built only from live reads.

## 3. Reconciliation pass (verified truth → drift → repair)
- [ ] 3.1 Add `reconcile(deps, runId, token, observeDeps)` in a new `core/scripts/loop/reconcile.ts`:
...
- [ ] 3.2 Classify each disagreeing item into exactly one `LoopDriftClass`; a drift with no/invalid
      class is unconstructable (runtime-guarded).
- [ ] 3.3 Repair **only** `ledger-behind` forward as an audited ledger transition (history entry +
...
- [ ] 3.4 Assert repair performs **no** external mutation — no merge, push, label write, or PR edit
      recorded through the injected seam.
- [ ] 3.5 Keep merge-barrier clearing behavior intact: the barrier clears only when the verified
...
- [ ] 4.1 On the durable transition path (`loop/store.ts` / `loop/recovery.ts`), refuse a transition
      ...
      unchanged.
- [ ] 4.2 Compose with (never bypass) the existing authority-gate and directly-verified-evidence
      requirements; local states (`implemented`) are unaffected.

## 5. Next-action computation
- [ ] 5.1 Add a pure `computeNextAction(item, identity, ledger)` returning one `LoopNextAction`; no
      clock, randomness, or I/O.
- [ ] 5.2 Surface the latest reconciliation and per-item next action in the read-only status
      projection (`getStatus`) — no writes, no lock, no GitHub call.

## 6. Modified durable-loop-engine
- [ ] 6.1 Remove the `Reconciliation SHALL record caller-observed truth and report drift without
...
## 7. Tests & gates
- [ ] 7.1 Fixture tests (injected seams, no real network/git/subprocess): identity build from live
...
- [ ] 7.2 Assert the reconciliation pass records **zero** real network/git/subprocess calls through
      the injected seam.
- [ ] 7.3 Prove each regression test bites (fails without the change).
- [ ] 7.4 Regenerate the plugin mirror (`node scripts/build.mjs`) and run `npm run ci` green,
