...
- At `pre-merge` specifically, budget exhaustion SHALL materialize the existing `ci-exhausted` / `implementation-ci` blocker (`autoLoopExhaustedBlockedOutcome` with `stage === "pre-merge"`) and SHALL `maybeReleaseWorktreeOnPark` so capacity is not stranded.
...
- [ ] When `finalStage` is `pre-merge`, the run materializes blocker kind `ci-exhausted` (reuse `autoLoopExhaustedBlockedOutcome(out, "pre-merge")`) and calls `maybeReleaseWorktreeOnPark` on that blocked outcome.
...
- **Shared recipe reuse:** `autoLoopExhaustedBlockedOutcome(out, stage)` already maps `pre-merge` → `ci-exhausted` (implementation-ci, not human-authority). Pre-merge budget death SHALL call that helper and `maybeReleaseWorktreeOnPark`. Do not add a new `BlockerKind`.
- **Events:** `core/scripts/run-store.ts` `RunCompleteEvent` / `finalizeRun`. Additive incomplete marker only; do not drop `run_complete` if that would hang logs-follow.
