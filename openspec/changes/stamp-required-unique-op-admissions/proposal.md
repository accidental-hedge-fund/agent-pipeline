...
- Make the existing control-host generic run-store admission path the shared contract for required public-entrypoint stamps, with a machine-checked inventory corresponding to `REQUIRED_PUBLIC_ENTRYPOINTS`.
...
- **Reuse first:** extend `persistPublicEntrypointAdmission`, `initRunDir`, the existing control-host root resolver, Logical Operation propagation, and the existing collector/mapper. Do not add a second run store, aggregator, recovery controller, or command-specific stamp implementation.
...
- **Affected implementation surfaces:** `core/scripts/run-store.ts`, public dispatch in `core/scripts/pipeline.ts`, train merge dependencies in `core/scripts/stages/train.ts`, unique-operation collection/classification, and their injected-seam unit tests.
