...
Issue #1343 is the factory-gate-v1 `clean-openspec` template for pack run
`pack-1400-pipeline-ship-1.40.0`. The pack needs one clean Pipeline path that
authors an OpenSpec change, lands a run-scoped fixture, verifies the fixture
names release `1.40.0`, and archives without a foreign active change.
...
- [ ] File `core/test/fixtures/frg/pack-1400-pipeline-ship-1.40.0/clean-openspec.json`
      exists and parses as JSON.
- [ ] That fixture names release `1.40.0` (exact string).
...
- [ ] This is the only active OpenSpec change for issue #1343.
