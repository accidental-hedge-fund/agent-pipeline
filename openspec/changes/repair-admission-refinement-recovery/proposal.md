...
- When eligible human feedback exists, require the OpenSpec revision producer to persist its genuine `## Human Feedback Acknowledgement` in the authoritative `proposal.md` as well as returning it in stdout, so the post-revalidation acknowledgement gate evaluates the same stable artifact used for publication and implementation. Pipeline must not synthesize the acknowledgement for the producer.
...
- [ ] With eligible human feedback, the OpenSpec revision prompt requires the producer to place the exact `## Human Feedback Acknowledgement` in both its response and authoritative `proposal.md`; a caller-level regression proves post-revalidation acknowledgement succeeds only when the stable validated proposal contains the producer-authored section, and Pipeline never synthesizes it.
...
- Close the remaining OpenSpec acknowledgement producer/consumer gap without weakening revalidation. `core/scripts/stages/planning.ts` establishes the current ordering: it validates revision stdout with `plan-revision.ack@1`, replaces `revisedPlan` with `revalidateArtifact(...).updatedPlanText`, then calls `validateHumanFeedbackAck` on that authoritative text. Therefore `core/scripts/prompts/plan_revision.md` must instruct the OpenSpec producer to persist its genuine acknowledgement in `proposal.md`, and a caller-level test must prove the revalidated stable proposal—not discarded stdout—passes the gate. Keep the completed stable-bundle, current-feedback, and freeform repairs intact and do not synthesize acknowledgements or redesign worktree recovery.
...
- OpenSpec revision instructions, durable producer-authored acknowledgement, current feedback handoff, artifact validation/reread, and implementation-plan construction in `core/scripts/stages/planning.ts`, `core/scripts/prompts/plan_revision.md`, and planning tests.
...
- @comamitc: addressed — retained the completed stable-artifact and current-feedback repairs, added the canonical linked-run and recovery replays and exported-writer overflow boundaries, and now requires the genuine acknowledgement to persist in `proposal.md` so the post-revalidation gate evaluates the authoritative artifact without synthesis or bypass.
