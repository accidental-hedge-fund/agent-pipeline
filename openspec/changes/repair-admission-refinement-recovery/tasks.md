...
- [ ] 2.3 Enforce the complete-body 65,536-character guard before the first mutation in non-dry-run `grillOneIssue`, MAC-valid `runRefineSpecApply`, and oversized-result `materializeGrillAnswer`; verify injected spies observe zero issue-body, label, handoff, frontier, sibling-rebind, and applicable recovery-receipt writes on refusal.
- [ ] 2.4 Replay the eleven-node repeated-evidence body and genuinely unique oversized bodies through the real exported admission writers; verify measurement includes unrelated issue text and rendered `## Decisions`, the compact case preserves every semantic field, and refusal truncates or omits nothing.
