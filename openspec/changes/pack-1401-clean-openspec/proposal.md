...
Issue #1425 is the factory-gate-v1 `clean-openspec` item for pack run
...
- `pack-1401-clean-openspec`: the run-scoped clean-openspec fixture for pack
  `pack-1401-pipeline-ship-1.40.1` names release `1.40.1`, and a unit test
...
- New files only: the run-scoped JSON fixture, one unit test under
  ...
  `openspec/specs/pack-1401-clean-openspec/`).
- Tests reuse `node:test` plus stdlib `node:fs` / `JSON.parse` (first
  holding rung: `core/test/version.test.ts` and the prior pack-1400
  clean-openspec test at `44a6e679`). No fixture loader, helper module,
  or registry.
...
- [ ] This change (`pack-1401-clean-openspec`) is the only active OpenSpec
      change for issue #1425. No foreign active change is introduced.
