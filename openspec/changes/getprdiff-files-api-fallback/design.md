...
3. **`patch` absent/empty and `changes > 0` (or additions/deletions > 0):** GitHub omitted a **text** patch. This is the PR #1222 hole (8 large deletes). **Do not** succeed with header-only. Materialize without a worktree:
   ...
   - `modified` / `changed` / `copied`, or `renamed` with `changes > 0`: files-entry `sha` is the **new** blob. Pin collection to one revision: read `gh api repos/${cfg.repo}/pulls/${prNumber}` (`base.sha` / `head.sha`) **before** listing files, list files, read the same pair **after** pagination, and compose only when the pairs match. Retry that before-list-after collection up to 3 times; fail closed if the PR is still moving. Derive merge-base from `gh api repos/${cfg.repo}/compare/{pinnedBase}...{pinnedHead}` → `.merge_base_commit.sha` of that captured pair — never from a later live PR read after the files list returns (that H1-blob + H2-merge-base mix is not a reviewable revision). Old blob: `gh api repos/${cfg.repo}/contents/<path>?ref=<merge_base_commit.sha>` → `.sha` (path slashes unencoded, same as `listPrHeadChangeDirs`; space/odd segments `encodeURIComponent` per segment), then git blobs. Compose a full-file replacement hunk (`@@ -1,oldCount +1,newCount @@`, all old `-`, all new `+`). No third-party diff library.
...
13. **H1→H2 collection race.** Files list first returns H1 blob SHAs; a later live PR read would be H2. Result is the stable H2 pair (after retry) or a moved-during-collection throw. **Must fail if H1 blobs compose with an H2 merge-base.**
