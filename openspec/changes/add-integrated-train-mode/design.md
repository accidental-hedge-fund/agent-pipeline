...
The Hermes factory pilot reimplemented one-item integrate waves in `ops/hermes-factory` (~12.5k LOC). That outer plane duplicated scheduling, journals, and proof, then failed on ownership and resume. Product direction (approved factory simplification plan) freezes that pilot as a harvest source and moves integrate trains into the Pipeline CLI.

Living specs already describe a `merged` item state and a merge barrier in the durable loop engine, but the engine does not call merge; something must supply merge evidence. Train mode is that explicit orchestrator.
