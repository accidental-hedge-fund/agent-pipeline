...
Issue #1342 is the Factory Reliability Gate (FRG) `clean-docs` item for
release `1.40.0` and pack run `pack-1400-pipeline-ship-1.40.0`. The pack
...
- The fixture SHALL name release `1.40.0` in field `release_version`.
- Add a unit test that reads only that run-scoped path and fails when
  `release_version` is not `1.40.0`.
...
- [ ] That fixture's `release_version` field is the string `1.40.0`.
...
- [ ] This change (`frg-pack-1400-clean-docs`) is the only active OpenSpec
      change for issue #1342. No foreign active change is introduced.
...
- `frg-pack-1400-clean-docs`: Run-scoped FRG `clean-docs` fixture for pack
  run `pack-1400-pipeline-ship-1.40.0`. The fixture at the run-scoped path
  SHALL name release `1.40.0`. A unit test SHALL verify that value.
