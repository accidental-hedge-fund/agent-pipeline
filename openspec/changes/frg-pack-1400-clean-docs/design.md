...
- Issue #1342 is a synthetic FRG `clean-docs` item. The pack run id is
  `pack-1400-pipeline-ship-1.40.0`. The release is `1.40.0`.
- `core/test/fixtures/frg/` does not exist on the issue base. The
  implementation creates that tree.
- Commit `7d801f4c` (`core/test/frg-pack-13916-clean-docs.test.ts`) is the
  holding reuse rung: `fileURLToPath(new URL("./fixtures/frg/<pack>/clean-docs.json",
  import.meta.url))`, `readFileSync`, `JSON.parse`, `assert.equal`.
...
Copy the pack-13916 disk-read `node:test` pattern. Use stdlib `node:fs`,
...
**Choice:** Add `frg-pack-1400-clean-docs` as a new capability. Do not
...
This issue's contract is a synthetic pack-run fixture that names `1.40.0`.
...
the string `1.40.0`. The unit test asserts that key only.

**Why:** The sibling FRG template `clean-openspec` names the same field.
The issue requires the fixture to name release `1.40.0`, not a full pack
manifest. Pack-13916 used this exact object.
...
**Choice:** Add `core/test/frg-pack-1400-clean-docs.test.ts`. The test
resolves
`core/test/fixtures/frg/pack-1400-pipeline-ship-1.40.0/clean-docs.json`
from `import.meta.url`, `JSON.parse`s the file, and asserts
`release_version === "1.40.0"`. It does not import `core/scripts/`.
...
3. Pre-merge archives `frg-pack-1400-clean-docs` into living specs. No
