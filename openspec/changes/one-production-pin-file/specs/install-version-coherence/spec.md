...
On the factory plane (`REPO_DIR` is the factory control checkout), `pipeline doctor` SHALL include an additive preflight check (stable id in the `install:` family) that compares `AGENT_PIPELINE_PRODUCTION_PIN` to `$REPO_DIR/.agent-pipeline/production-engine-pin.json` when the env is set to a different resolved path. When both files are readable and their `version` or `git_sha` disagree, the check SHALL have status `"fail"`. The check SHALL NOT use `"warn"` or `"pass"` for that disagreement. Remediation SHALL name both paths and instruct the operator to unset the env (so Tugboat binds the control pin) or to point the env at the control-checkout pin. When the env is unset, when both paths resolve to the same file, or when `version` and `git_sha` agree, this check SHALL NOT fail for split-pin disagreement. Ordinary non-factory product repositories SHALL skip this check. A unit test SHALL fail if env pin and control pin disagree and the result is pass. The check SHALL obtain pin contents through injected `DoctorDeps` file-read primitives so unit tests perform no real filesystem, network, git, or subprocess calls.
