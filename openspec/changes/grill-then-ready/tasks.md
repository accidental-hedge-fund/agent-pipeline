...
- [x] 4.4 Reject Implementer output that marks its own nodes `accept` / `settled-by: reviewer-accept` / `resolution: resolved` before the Reviewer call, require an exact unique Reviewer verdict per Implementer-proposed node, and verify no GitHub mutation and no Reviewer invoke on self-accept or omitted verdict
