...
**Choice:** Keep `trainRunIdFor(startedAt)` as `train-<filesystem-safe UTC ms>`. Allocate by creating `.agent-pipeline/runs/<id>/` with `mkdir` and `recursive: false` (EEXIST is a collision). On collision, retry `train-<timestamp>-2`, `-3`, … up to 8 attempts through injected mkdir and clock/ID seams. After a unique directory is created, call existing `initRunDir` for `run.json` / `events.jsonl`. Do not treat a colliding train directory as idempotent re-entry of a different train. Do not change advance `initRunDir` when `run.json` already exists for the same issue-prefixed id.
