...
- Issue #1291 is a synthetic FRG `clean-openspec` item. The pack run id is
  `pack-13915-pipeline-ship-1.39.15`. The release is `1.39.15`.
...
**Choice:** Add `frg-pack-13915-clean-openspec` as a new capability. Do not
...
This issue's contract is a synthetic pack-run fixture that names `1.39.15`.
...
- `skip_specs: true` → rejected. The issue requires one OpenSpec requirement
  that the fixture names `1.39.15`.
...
the string `1.39.15`. The unit test asserts that key only.

**Why:** The sibling FRG template `clean-docs` names the same field. The
issue requires the fixture to name release `1.39.15`, not a full pack
manifest.
...
**Choice:** Add `core/test/frg-pack-13915-clean-openspec.test.ts` (or an
...
`release_version === "1.39.15"`. It does not import `core/scripts/`.
...
3. Pre-merge archives `frg-pack-13915-clean-openspec` into living specs. No
