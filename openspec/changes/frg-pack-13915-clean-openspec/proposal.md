...
Issue #1291 is the Factory Reliability Gate (FRG) `clean-openspec` item for
release `1.39.15` and pack run `pack-13915-pipeline-ship-1.39.15`. The pack must
...
- The fixture SHALL name release `1.39.15` in field `release_version`.
- Add a unit test that reads only that run-scoped path and fails when
  `release_version` is not `1.39.15`.
...
- [ ] That fixture's `release_version` field is the string `1.39.15`.
...
- [ ] Production files under `core/scripts/`, `hosts/`, and `plugin/` are
      unchanged.
- [ ] This change (`frg-pack-13915-clean-openspec`) is the only active
      OpenSpec change for issue #1291. No foreign active change is introduced.
...
- `frg-pack-13915-clean-openspec`: Run-scoped FRG `clean-openspec` fixture for
  pack run `pack-13915-pipeline-ship-1.39.15`. The fixture at the run-scoped
  path SHALL name release `1.39.15`. A unit test SHALL verify that value.
...
- **OpenSpec:** this change only. Pre-merge archives it into
  `openspec/specs/frg-pack-13915-clean-openspec/spec.md`.
