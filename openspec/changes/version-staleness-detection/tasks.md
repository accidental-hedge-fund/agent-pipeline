...
- [x] 5.2 In `scripts/pipeline-launcher.mjs`, move the `!pkgReadable` guard ahead of the `core/node_modules` check so a corrupt install that also lacks dependencies still reports version-coherence
- [x] 5.3 Tests (`scripts/launcher-smoke.mjs`): malformed package.json with `doctor --json` (valid envelope), `doctor --is-ok` (silent, non-zero), malformed + no `node_modules` (version-coherence, not deps error), and the host shim's `--json`/`--is-ok` paths
