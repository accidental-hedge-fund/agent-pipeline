...
- [ ] 1.1 Add a shared parallel-pilot fixture builder (co-located with the simulation test) that
...
- [ ] 1.2 Provide a scripted scheduler / `SupervisorDeps` execution fake whose per-cycle outcome
...
- [ ] 2.1 Drive a scheduling pass and assert A and B are admitted into the concurrent set together
      (budget honored, pairwise `disjoint`), each assigned its **own separate managed worktree**.
- [ ] 2.2 Assert each item retains independent Pipeline evidence — one member's ledger history,
...
- [ ] 3.1 Assert item C is excluded from the concurrent set (serialized to run after the active
      items drain).
- [ ] 3.2 Assert C's durable planning record carries exactly **one** structured conflict reason
...
- [ ] 4.1 Flip the observation fake so concurrently-run A and B are seen to have actually changed an
...
- [ ] 4.2 Assert neither parked item proceeds into concurrent merge preparation, an unaffected
...
- [ ] 5.1 After the concurrent implementation/review work, assert merge / base refresh / final
...
- [ ] 5.2 Assert each admitted item still passes its own review and pre-merge gates and the run
      still stops at `pipeline:ready-to-deploy` (the scheduler grants no merge authority).

## 6. Evidence bundle
- [ ] 6.1 Emit one evidence bundle for the pilot run referencing: the observed concurrency (which
...
- [ ] 6.2 Assert the bundle is derived from recorded run state (planning record / ledger / events /
...
- [ ] 7.1 Compose 2–6 into a single end-to-end simulation test driving the
...
- [ ] 7.2 Assert the simulation records **zero** real network, git, and subprocess calls.
- [ ] 7.3 Prove each assertion bites: temporarily defeat each composed behavior (concurrent
...
- [ ] 8.1 Write a live-pilot runbook (operator-facing) with the exact steps to run the real
...
- [ ] 8.2 Enumerate the evidence-bundle artifact contract the completed live run must capture to be
...
## 10. Gates
- [ ] 10.1 Regenerate the plugin mirror (`node scripts/build.mjs`) and run `npm run ci` green,
