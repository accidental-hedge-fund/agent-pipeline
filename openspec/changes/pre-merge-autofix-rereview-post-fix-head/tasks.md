...
- [x] 1.1 Change `performPreMergeAutoFix` to return the post-fix commit SHA on success
      (e.g. `{ status: "fix-committed", headSha }` or a tuple), read from local git state
      after the amend — not from a GitHub-API PR-head read.
- [x] 1.2 Update the `AttemptPreMergeAutoFixFn` seam type and `PreMergeAutoFixResult` to
      surface the post-fix head, keeping the `"error"` result shape intact.
- [x] 1.3 Update the production wiring that constructs the `attemptPreMergeAutoFix` seam so
      the returned head flows to the caller.

## 2. Anchor the re-review to the post-fix head

- [x] 2.1 In the `enforceReviewShaGate` re-review branch, use the auto-fix's returned
      post-fix head as `newPrHead` instead of `getPrDetail(...).head_sha`.
- [x] 2.2 Compute the re-review delta diff over `reviewed.sha...<post-fix-head>` from a git
...
- [x] 2.3 Preserve the existing fall-through: if the post-fix head is unavailable or its
...
- [x] 2.4 Keep the one-attempt bound and the review-2 ceiling exemption unchanged.

## 3. Regression tests (deps seam, no real I/O)

- [x] 3.1 Add a test driving the fix-then-re-review path that asserts the second review
...
- [x] 3.2 Prove the test bites: with the post-fix head resolved from the stale API read,
      the test fails.
- [x] 3.3 Confirm the existing auto-fix tests (advance-on-resolve, escalate-on-unresolved,
