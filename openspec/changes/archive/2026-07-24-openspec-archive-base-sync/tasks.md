...
- [x] 1.1 In `maybeArchiveOpenspec` (`core/scripts/stages/pre_merge.ts`), before running
...
- [x] 1.2 Capture the reviewed/pushed head SHA (`origin/<branch>` after fetch) and the
      worktree `HEAD` SHA as `archiveBase`.

## 2. Base-equality gate

- [x] 2.1 After the sync attempt, compare worktree `HEAD` to `origin/<branch>`. If they
      are equal, proceed to archive/commit as today.
- [x] 2.2 If they are not equal (fast-forward impossible / true divergence), block with
...
- [x] 3.1 Keep the existing block-on-non-fast-forward-push behavior; ensure no code path
      adds `--force` / `--force-with-lease` to the archive push.
- [x] 3.2 Add an inline comment documenting that a non-fast-forward archive push is a
      block signal, never a force-overwrite, and cite #579.

## 4. Tests (co-located, via `AdvancePreMergeDeps` seam — no real network/git)

- [x] 4.1 Behind-remote sync: worktree `HEAD` is an ancestor of `origin/<branch>`; assert
      the step fetches + fast-forwards, archives on the reviewed head, and the resulting
      push is fast-forwardable (no block, no force).
- [x] 4.2 Diverged base: worktree `HEAD` and `origin/<branch>` share only an older
...
- [x] 4.3 Already-in-sync (regression-guard): worktree `HEAD` already equals
      `origin/<branch>`; assert behavior is unchanged (archive → commit → normal push).
- [x] 4.4 Prove each test bites — it fails against the pre-fix code.

## 5. Mirror + gate

- [x] 5.1 `node scripts/build.mjs` and commit the regenerated `plugin/`.
- [x] 5.2 `openspec validate openspec-archive-base-sync` passes; `npm run ci` green.
