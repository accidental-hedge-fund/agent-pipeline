...
- [x] 1.1 In `core/scripts/evals/fixture.ts`, add an optional `environment` list to the fixture
      ...
      `setup`/`teardown`.
- [x] 1.2 Extend fixture validation to reject an unknown `mode`, or a dependency missing a required
...
- [x] 1.3 Bump `schema_version` handling if needed so the new field is additive: fixtures without an
...
- [x] 2.1 Encode the default-mode rule: a newly declared dependency defaults to `simulated` (when a
      deterministic stand-in is possible) or `forbidden`; `live` is never the default.
- [x] 2.2 Require an explicit maintainer selection to set a dependency `live` when it can incur
...
- [x] 3.1 Derive an environment-and-surface provenance hash over the resolved environment-fidelity
      contract plus the resolved capability-surface inventory; expose it off the loaded fixture.
- [x] 3.2 In `core/scripts/evals/run.ts` / `results.ts`, record that hash on every cell record
...
- [x] 4.1 Add `core/scripts/evals/harvest.ts`. Accept one or more evidence references: normal run
      artifacts, `pipeline improve` clusters, or `correction_event` / control proposals (#499/#500).
- [x] 4.2 Run every evidence excerpt through the existing `artifact-sanitize` path (secret redaction
      + injection screen) before it can enter a draft or proposal body; never copy raw production
      payloads or secrets.
- [x] 4.3 Handle missing/empty evidence explicitly with a clear error rather than emitting a
      degraded draft.

## 5. Capability-surface inventory + single-ability proposal

- [x] 5.1 Resolve and emit a capability-surface inventory for the candidate: stage, materialized
      prompts, harness/model configuration, tools/hooks, repository paths, referenced services/data
      dependencies.
- [x] 5.2 Propose exactly one bounded ability or failure mode; record source evidence, affected
...
- [x] 6.1 Render a draft conforming to the #432/#433 fixture contract: immutable `base_commit`, task
...
- [x] 6.2 Confirm the rendered draft loads under the existing fixture loader.

## 7. Iterative revision + promotion authority boundary

- [x] 7.1 Support iterative maintainer revision of the proposed ability, task, dependency modes,
      checks, and grader; re-render a consistent draft after each edit.
- [x] 7.2 Keep draft-only the default. Repository writes require an explicit `--apply`/`--promote`
...
- [x] 7.3 On promotion, validate the draft with the existing eval loader (reject an invalid draft by
...
- [x] 8.1 Add a `pipeline evals harvest` subcommand dispatch in `core/scripts/pipeline.ts`
      (draft-only default; explicit `--apply`/`--promote`).
- [x] 8.2 If gating is needed, add a fixtures/harvest config block in `core/scripts/config.ts`
      (e.g. default-mode policy) that rejects unknown keys.

## 9. Tests + mirror + CI

- [x] 9.1 Fixtures + tests covering: harvested correction evidence, ordinary run-failure evidence,
      ...
      change.
- [x] 9.2 Test the environment-contract validation (unknown mode / incomplete dependency rejected by
...
- [x] 9.3 `node scripts/build.mjs` to regenerate the `plugin/` mirror; run `npm run ci` from root
