...
- [ ] An explicit candidate-SHA override from advance `--sha <40-hex>` is accepted as the readiness subject when it is a full 40-hex SHA and, if a PR exists, matches that PR head. A bogus override is refused with a named outcome.
