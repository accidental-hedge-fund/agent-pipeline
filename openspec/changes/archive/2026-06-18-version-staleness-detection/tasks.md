...
- [x] 5.0 In both launchers, classify `core/package.json` as corrupt (`pkgReadable = false`) not only when read/parse throws, but also for valid JSON that still prevents the ESM-only `.ts` entries from loading: a non-object, a non-string `version`, or an explicit `type` other than `"module"` — `type: 123` trips `ERR_INVALID_PACKAGE_CONFIG`; `type: "commonjs"` makes the entries' ESM `import`s fail (absent `type` is healthy → ESM)
