...
- [ ] 1.1 Define the fleet envelope wrapping the existing sanitized `appendEvent` record: bounded
      ...
      a high-entropy `run_id` generated once per run and durably persisted before the first event,
      event `schema_version`, explicit `envelope_version`, and per-run monotonic `seq`.
...
- [ ] 2.3 Implement at-least-once delivery with a per-envelope deterministic idempotency key embedded in
...
- [ ] 3.1 Implement the versioned wire transport profile (design.md D10): authenticated HTTPS ingest
      endpoint, canonical field types/limits, per-envelope idempotency-key encoding, `202`/`4xx`
      single-envelope acknowledgement/rejection, `207 Multi-Status` per-envelope batch results, and a
      `/fleet/v1/capabilities` endpoint.
...
- [ ] 3.4 Deterministically deduplicate on the idempotency key so duplicate delivery does not skew
      metrics; reconstruct per-run order from `(run_id, seq)`; flag (never silently merge) a
      duplicate-`run_id` conflict within an installation.
- [ ] 3.5 Write to customer-owned storage; assert there is no upstream path to Agent Pipeline
      maintainers.
- [ ] 3.6 Implement the authenticated query/report endpoint (design.md D11): `POST /fleet/v1/query` with
      scoped-credential tenant resolution, `time_window`/`filter`/`group_by`/`page` request handling,
      paginated `metrics`/`lineage`/`next_cursor` response, and cross-tenant-filter refusal.
- [ ] 3.7 Tests: capabilities/ack/rejection wire-format conformance (including batch partial-success),
      multi-tenant/multi-repo/multi-host ingest, out-of-order and duplicate events, duplicate-run-id
      conflict detection across hosts, malformed + unsupported-version rejection, unknown-optional-field
      acceptance, cross-tenant write/query/query-filter refusal — each biting.

## 4. Read-only fleet reporting

- [ ] 4.1 Add a read-only `pipeline fleet` report (human + JSON) that queries the collector's
      `POST /fleet/v1/query` endpoint (design.md D11) with a scoped query credential and aggregates the
      existing scoreboard/improve metrics across authorized repos/hosts; assert no GitHub/worktree/
      config/run-artifact mutation.
