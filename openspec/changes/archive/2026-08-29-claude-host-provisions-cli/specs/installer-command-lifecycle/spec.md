...
**Reason:** `install --host claude` no longer writes `pipeline:<name>.md` files. There is no invoke-line path to embed.

**Migration:** The SKILL overlay lives under the resolved Claude config base (`CLAUDE_CONFIG_DIR` when set). Operators exec `pipeline <verb>`. Uninstall still removes leftover `pipeline:*.md` from older installs (see the modified uninstall requirement).
