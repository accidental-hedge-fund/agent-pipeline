...
**Reason:** The committed `plugin/` core mirror is retired by #1048, so the old requirement identity describes a packaging contract that no longer exists.

**Migration:** Repo-local instructions require SKILL/catalog freshness after core edits and forbid requiring a committed `plugin/` core copy.

### Requirement: Repo-root golden rule 1 SHALL name CLI plus SKILL as the product until the mirror is retired

**Reason:** The mirror is retired by #1048, so the transitional requirement identity is no longer durable.

**Migration:** The golden rule names CLI plus SKILL as the product and describes the remaining build gate as SKILL/catalog freshness.
