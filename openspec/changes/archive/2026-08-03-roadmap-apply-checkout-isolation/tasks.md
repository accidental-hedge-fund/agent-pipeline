...
- [x] 1.1 Extend `WritebackDeps` in `core/scripts/roadmap/writeback.ts` with injectable
...
- [x] 1.2 Wire the real implementation from the roadmap orchestrator / real-deps
...
- [x] 1.3 Remove operator-checkout branch switching from the roadmap PR path: stop
...
- [x] 2.1 Refactor `openRoadmapPr` so all docs file write, commit, and push run inside
      the throwaway worktree path, not the operator `repoDir`.
- [x] 2.2 When `findPrByHead` finds an existing PR: compare rendered markdown to
...
- [x] 2.3 When no PR exists: create/update branch content in the throwaway worktree,
      commit, push, `createPr` as today (still no operator branch switch).
- [x] 2.4 Replace the hardcoded commit message fossils (`Issue: #171`,
...
- [x] 3.1 Replace the `roadmap-writeback.test.ts` regression that **requires**
...
- [x] 3.2 Add existing-PR refresh tests: content differs → commit+push, no `createPr`;
      content identical → no commit; comparison uses branch-head content from the
      worktree seam, not only the operator working tree.
- [x] 3.3 Add commit-message shape tests: no fossil `#171` / fake run id strings; with
      both issue+run context → both trailers present; with partial/no context → no
      invented trailers.
- [x] 3.4 Prove the isolation and fossil-message tests bite against the pre-fix behavior
...
- [x] 4.1 Run `node scripts/build.mjs` and include the regenerated `plugin/` mirror in
      the same change as any `core/` edits.
- [x] 4.2 Run `npm run ci` from the repo root and fix failures until green (core tests,
