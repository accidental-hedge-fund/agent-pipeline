...
**Specificity**: Two different issues at the same file + 5-line band + severity map to the same key. In practice, a code reviewer almost never flags two separate blocking issues of the same severity at the same location (a 5-line band is narrow). When it does occur, `partitionFindings` detects the collision (two findings share the same key in the current verdict) and withholds the override from both — they remain blocking. This prevents an operator from inadvertently suppressing a distinct bug they never reviewed. The operator must widen the scope of their override (e.g., use `--override` on a later verdict after the two findings are distinguishable) or fix one of the findings to resolve the collision.
...
**Rationale**: A 5-line band is narrow enough to avoid collapsing findings across distinct code constructs, while wide enough to absorb the ±1–2 line drift typical when a fix round edits the surrounding code (a surrounding method signature changes, an import is added, etc.). Within a single bucket the maximum observable drift is 4 lines (from the first to the last line of the band). A drift that crosses a bucket boundary produces a different key — the override does not survive this case. This is intentional: a finding that drifts more than a few lines typically indicates a genuine structural change that warrants re-evaluation.
