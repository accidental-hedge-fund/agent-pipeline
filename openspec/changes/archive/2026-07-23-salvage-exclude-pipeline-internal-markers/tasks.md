...
- [x] 1.1 In `core/scripts/salvage-harness-work.ts`, add an exported canonical constant for
...
- [x] 1.2 In `core/scripts/stages/pre_merge.ts`, make `REBASE_MARKER_FILE` refer to the canonical
...
- [x] 2.1 In `defaultGitStatus` (unscoped) and the scoped status branch, restrict
...
- [x] 2.2 Confirm a worktree whose only dirty path is the marker yields empty (clean) status →
...
- [x] 3.1 Fold the marker exclusion pathspecs into the unscoped `SALVAGE_GIT_ADD_ARGS` alongside
      `SALVAGE_NODE_MODULES_EXCLUDE`.
- [x] 3.2 Fold the same marker exclusion into the scoped add-args branch, preserving the existing
      `openspec/` scope and node_modules exclusion.

## 4. Tests (prove they bite)

- [x] 4.1 `core/scripts/salvage-harness-work.test.ts`: dirty worktree whose only path is
...
- [x] 4.2 `core/scripts/salvage-harness-work.test.ts`: dirty worktree with a real changed file
...
- [x] 4.3 `core/scripts/salvage-harness-work.test.ts`: the scoped (`openspec/`) salvage still stages
      only `openspec/` and additionally excludes the marker; existing scoped tests still pass.
- [x] 4.4 Drift-guard test: assert `REBASE_MARKER_FILE` (pre_merge) equals the canonical
      `PIPELINE_INTERNAL_MARKER_FILES` entry, so the two cannot diverge.

## 5. Mirror + CI

- [x] 5.1 `node scripts/build.mjs` — regenerate the `plugin/` mirror; commit it in the same change.
- [x] 5.2 `npm run ci` passes green from the repo root (core tests, mirror check, install smoke,
