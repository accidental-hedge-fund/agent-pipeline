...
Today four hand-maintained surfaces restate overlapping operator knowledge:

| Surface | ~lines (this worktree) | Jobs |
| --- | --- | --- |
| `README.md` | ~2168 | Landing + install + ~20-command CLI reference + config guide + concepts |
| `hosts/claude/SKILL.md` | ~1335 | Host skill + Modes command table + config + run flow |
| `hosts/codex/SKILL.md` | ~1126 | Same for `$pipeline` |
| `ROADMAP.md` | ~883 | Forward plan + accreting "Shipped" prose + release-plan table |
...
- **Releases:** git tags + GitHub Releases; `core/scripts/stages/release.ts` currently mutates ROADMAP (intro chain, Shipped block, plan row).
...
Infrastructure **already on main** that this change activates (must not reinvent):

- `scripts/ci-docs.mjs` — no-op without `scripts/generate-docs.mjs`; real check-mode when present.
- `docs-freshness-gate` — pre-PR docs check + auto-heal when generator present.
- `ci-checkout-generator-parity` — PR/main CI full history + tags for tag-sourced CHANGELOG.
- Implementing prompt language already names regenerate of generator outputs when present.

Related but **separate** work: #626 (stage-count SSOT / TERMINAL alignment) and #598 (published docs site). Prior implement PR #711 closed without merge; this design incorporates its useful decisions without re-landing broken intermediate state.
...
3. CI **fails closed** when committed generated docs are stale (mirror-style via `generate-docs.mjs --check`, wired through existing `ci:docs`).
4. README becomes a **landing page** with links into `docs/`; target well under ~400 lines.
5. ROADMAP stops accreting shipped history; history lives in **`CHANGELOG.md`**; release tooling stops growing ROADMAP Shipped prose as the primary notes surface.
...
- Auto-merging, full release automation redesign, or changing `pipeline config schema` wire format.
- Making every registry keyword user-facing (hidden/legacy aliases stay dispatch-only).
- Replacing host SKILL procedural run-flow content (only the command inventory region is generated).

## Decisions

### D1 — Single generator entry point: `scripts/generate-docs.mjs`

**Decision:** One Node entry point at `scripts/generate-docs.mjs` that:

- **Write mode (default):** regenerates all generator-owned artifacts and writes them in place.
- **Check mode (`--check`):** computes the same outputs and exits non-zero if any committed file would change, listing stale paths (format compatible with `extractStalePaths` in `docs-freshness.ts`: a `stale generated docs:` block with bullet paths).

Core transform logic MAY live under `core/scripts/` (e.g. `docs-generate.ts`, `command-docs.ts`) imported via native type-stripping so unit tests co-locate with other core tests. The scripts entry point remains the CI/operator surface so `ci-docs.mjs` / docs-freshness detection stay correct without path changes.

**Why not only core unit tests that write docs?** Fails "contributor runs one command to refresh" and CI `--check` ergonomics. **Why not shell/markdown outside Node?** Cannot import the TypeScript registry/schema cleanly.
...
**Coverage:** Document top-level keys and important nested blocks (e.g. `review_policy`, `steps`, `eval_gate`, `doctor`, `harnesses`) with type, default where known from `DEFAULT_CONFIG`, and description. Explicitly **omit** keys the schema rejects (e.g. `auto_merge`).
...
### D5 — Staleness gate (mirror-style) via existing ci:docs
...
Wire package scripts such as `docs:generate` / `docs:check` if useful for contributors, but the hard contract is:

1. File `scripts/generate-docs.mjs` exists (activates `ci-docs.mjs` and docs-freshness-gate).
2. `--check` is real check-mode (not write-mode under a check name).
3. Root `npm run ci` already includes `ci:docs` — once the generator is present, that step becomes a real freshness gate without redesigning the ci chain.

Comparison is content-based (normalize trailing newlines; prefer **no** unstable timestamps in output). Generated files commit to git (like `plugin/`), so clones and GitHub render work without running the generator.
...
2. **Release tooling hook:** `pipeline release` SHALL stop growing free-form ROADMAP "Shipped" prose as the primary notes surface. Prefer refreshing/appending the newest CHANGELOG section (or running the docs generator) as part of the release path when low-risk; at minimum, stop calling `prependShippedBlock` / intro-chain history accretion that reintroduces the unbounded prose. Keep compact release-plan table updates (✅ shipped row) if still useful for forward planning.
...
- **Staleness:** `--check` covered by tests and by running the real check in `npm run ci` once the generator is present (CI is the production gate; docs-freshness-gate covers pre-PR).
...
- **Release path:** regression that release ROADMAP mutations do not reintroduce accreting Shipped prose as the history surface.

### D10 — Compatibility with docs-freshness / ci-docs contracts

**Decision:** Do not redesign presence detection. Match the existing contracts:

- Generator path: `scripts/generate-docs.mjs`
- Stale output shape: list paths under a `stale generated docs:` banner so `extractStalePaths` works
- Optional npm scripts: `docs:generate` / `docs:check` (check-mode body preferred for `docs:check`)

When this lands, `ci:docs` flips from no-op to real check automatically; implement/test-gate auto-heal becomes active for generator-present worktrees.
...
| CHANGELOG backfill is noisy or incomplete for early tags | Prefer tag subject + release body when present; allow sparse early entries over inventing prose. Full CI checkout already provides tags. |
...
| Release tooling still writes ROADMAP Shipped after generator lands | Explicit task to retire/redirect `prependShippedBlock` / intro-chain history; regression test. |
| Prior PR #711 left no main-line artifacts but trained reviewers on pitfalls | Keep generator activation contract aligned with #716/#756; prove local + CI check parity with tags present. |

## Migration Plan

1. Land generators + metadata + first generated `docs/cli.md` / `docs/config.md` while README still has old content (single PR is fine).
2. Replace host SKILL command regions with markers + generate.
3. Rewrite README landing; extract concepts.
4. Generate CHANGELOG; strip ROADMAP Shipped prose; add pointer; adjust release mutations.
5. Confirm `ci:docs` becomes a real check; prove red on intentional staleness.
...
2. **Whether `pipeline release` must write CHANGELOG in this change** — preferred yes if the hook is small; if release path is risky, the minimum is stop accreting ROADMAP Shipped prose and document a manual `generate-docs` step in the release checklist as interim.
