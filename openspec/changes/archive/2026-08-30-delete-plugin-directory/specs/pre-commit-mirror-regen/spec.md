...
**Reason:** After #1050 the generator no longer owns a plugin SKILL overlay or a `plugin/`-sourced marketplace catalog, so the old requirement identity names retired outputs.

**Migration:** The hook refreshes and stages only the four generated host SKILLs.
