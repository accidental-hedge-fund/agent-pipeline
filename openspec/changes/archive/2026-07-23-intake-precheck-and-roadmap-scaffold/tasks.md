...
- [x] 1.1 In `core/scripts/stages/intake.ts` `runIntake`, split the deterministic,
...
- [x] 1.2 Run all of 1.1 BEFORE `d.runHarness` is called, so a failing precondition aborts
...
- [x] 2.1 Add a helper (in `core/scripts/stages/release.ts`, next to
...
- [x] 2.2 In `applyRoadmapMutations`, when the target-release detail section is missing,
...
- [x] 2.3 Define the degrade fallback: if the structure genuinely cannot be scaffolded,
...
- [x] 2.4 Ensure `--dry-run` prints the diff including the scaffolded heading and writes
  nothing.

## 3. Tests

- [x] 3.1 Deps-seam regression: a failing precondition (remove a global table anchor from
...
- [x] 3.2 Replace the current "detail section absent → throws" test
  ...
  an issue is created.
- [x] 3.3 If the degrade fallback is implemented, cover it: issue created + gap report
  emitted + spec not discarded.
- [x] 3.4 Unit test the new scaffold helper directly (heading inserted at the right place;
  idempotent when the section already exists).

## 4. Mirror & gate

- [x] 4.1 Run `node scripts/build.mjs` and commit the regenerated `plugin/` mirror.
- [x] 4.2 `npm run ci` green (core tests, mirror `--check`, install smoke, openspec
  validate).
