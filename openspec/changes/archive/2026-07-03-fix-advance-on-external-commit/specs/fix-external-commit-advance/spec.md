...
The fix stage SHALL block with the existing `no-commits` blocker and reason, unchanged from prior behavior, when a fix round's harness produces no new commit, salvage finds nothing, and the worktree `HEAD` equals the reviewed SHA — because no work has been done since the reviewer last reviewed the branch.

#### Scenario: No new commit, salvage empty, HEAD equals reviewed SHA — blocks as before
