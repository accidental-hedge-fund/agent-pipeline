...
- [x] 1.1 Choose one form: (A) single-line/bullet provenance note under `docs/`
...
- [x] 1.2 Artifact wording/fields MUST name: clean composition pack item **1**,
...
- [x] 1.3 If form (B): add a hermetic `node --test` unit test that loads only that
      fixture path and asserts `release_version === "1.34.0"`; prove the assertion
      bites by temporary mutation during development
      (N/A — form A docs note chosen; no fixture/test required)
- [x] 1.4 Confirm the implementation diff does not edit
      `core/scripts/factory-reliability-gate.ts`, release preflight, scoreboard,
      thresholds, or product feature code

## 2. OpenSpec hygiene

- [x] 2.1 Keep this change folder complete (`proposal.md`, `design.md`,
      `tasks.md`, `specs/factory-reliability-gate/spec.md`)
- [x] 2.2 Run `openspec validate frg-pack-clean-composition-item-1-v1-34-0` and
      fix any structural errors
- [x] 2.3 Do not create a second OpenSpec change for issue #959

## 3. Verification

- [x] 3.1 Run `npm run ci` from the repo root and ensure green
- [x] 3.2 Spot-check acceptance criteria in `proposal.md` against the landed
