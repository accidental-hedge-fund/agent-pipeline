...
Provides `pipeline config validate` — a never-throws CLI command that validates `.github/pipeline.yml` and emits severity-tiered structured JSON diagnostics. External tools (Pipeline Desk, editor integrations) call this command to get actionable, machine-readable feedback before saving a config file, without duplicating validation logic.
