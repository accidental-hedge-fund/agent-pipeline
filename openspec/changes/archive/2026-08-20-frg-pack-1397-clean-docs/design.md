...
See `proposal.md` for motivation. Issue #1176 is the `clean-docs` template instance for FRG pack `pack-1397-tugboat-ship-1.39.7`. `core/test/fixtures/frg/` does not exist yet. Existing unit tests under `core/test/` read JSON with `node:test` plus `node:fs` and inject no network, git, or subprocess.
...
- Land one unit test that pins `release_version` to `1.39.7` from that path only.
...
1. **Hard-code the pack-run path.** The issue names `core/test/fixtures/frg/pack-1397-tugboat-ship-1.39.7/clean-docs.json`. The test reads that path. It does not glob sibling packs. Alternative: a shared loader keyed by `pack_run_id`. Rejected because this item must stay run-scoped and production-inert.
...
- [Sibling #1177 also authors an OpenSpec change] → Keep this change id unique (`frg-pack-1397-clean-docs`) and do not share the fixture path with `clean-openspec.json`.
