...
- [x] A dispatched item observed on live truth carrying `pipeline:blocked` — including when
...
- [x] Detection uses the label's presence in the item's live label set, not whether
      `pipelineStageFromLabels` returns `blocked` as the single stage-winner.
- [x] A per-item hold does NOT by itself terminate the run: while any other item can make
...
- [x] The run reaches its terminal outstanding-hold condition only when no non-done item can
      make progress; that terminal report enumerates every held item id in the durable record
      and the `pipeline loop` CLI output.
- [x] A genuine engine defect (rejected/crashed dispatch, or unrecognized terminal outcome
      with the item at no `pipeline:blocked` state) is still `workflow-engine-defect` /
      `run_fatal`.
- [x] Regression test: one already-`pipeline:blocked` item + N clean items → the N clean
...
1. [x] Add a presence predicate for the `pipeline:blocked` disposition in
...
2. [x] In supervisor Pass 2 (`core/scripts/loop/supervisor.ts`), replace the needs-human
...
3. [x] Change the cycle-start `held` short-circuit in `runSupervisorCycle` so a held
...
4. [x] Record the terminal outstanding-hold condition (pause, `hold_outstanding=true`) only
...
5. [x] Surface the held item ids in the `pipeline loop` result JSON emitted by
...
6. [x] Tests (co-located `*.test.ts`, dependency-seam fakes, no real network/git/subprocess):
       - [x] Regression: one item carrying `pipeline:blocked` co-present with a stage label
             ...
             recorded.
       - [x] Presence detection: a `pipeline:blocked` label co-present with a `pipeline:*`
             stage label routes to the needs-human hold (fails under the pre-fix
             stage-winner guard).
       - [x] Continuation: a held item alongside a schedulable sibling does not pause the
             run; the sibling is dispatched (fails under the pre-fix `held` short-circuit).
       - [x] Terminal hold: when every remaining item is held/blocked, the run reaches its
...
7. [x] Run `node scripts/build.mjs` to regenerate the `plugin/` mirror; commit it in the
       same change.
8. [x] Run `npm run ci` from repo root; treat red as not-done.
