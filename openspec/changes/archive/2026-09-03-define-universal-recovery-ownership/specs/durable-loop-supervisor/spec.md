...
A live drive that first records `run_fatal` SHALL enter Cooling or an external-condition wait under
`recovery-lifecycle-ownership`. The supervisor SHALL NOT auto-retry that fault without a later wake
(operator `--resume`, cooling eligibility, or an external-condition probe). Historical
`stop.reason = run_fatal` MAY remain as a compatibility projection. That record SHALL NOT end
Logical Operation ownership or grant human authority.
...
- **AND** that dispatch records a new `run_fatal` compatibility projection
- **THEN** the new record's `time` SHALL differ from the superseded stop's `time`
...
- **WHEN** a live supervisor (not a resume of an already-recorded `run_fatal`) records a
  `run_fatal` class under recovery policy
- **THEN** it MAY persist `stop.reason = run_fatal` as a compatibility projection
- **AND** the Logical Operation SHALL enter Cooling or an external-condition wait
...
- **WHEN** `--resume <run-id>` targets a run stopped with `reason = run_fatal`
- **AND** every remaining non-done item is under a current typed-input wait
