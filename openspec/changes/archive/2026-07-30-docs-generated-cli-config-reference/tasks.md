...
- [x] 1.2 Mark legacy/hidden keywords (for example `run`) as undocumented so generators omit them
...
- [x] 4.1 Provide `node scripts/generate-docs.mjs` (name may vary) with write mode and `--check` mode covering `docs/cli.md`, `docs/config.md`, SKILL generated regions, and generator-owned `CHANGELOG.md` if applicable
- [x] 4.2 Wire the check into root `npm run ci` (or a documented sub-script that `ci` invokes)
...
- [x] 6.5 Optionally hook `pipeline release` / release workflow to refresh the newest section if low-risk; otherwise document the regenerate step for release operators
...
- [x] 7.2 Run `openspec validate docs-generated-cli-config-reference` and fix structural issues
