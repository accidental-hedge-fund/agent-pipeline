...
- [x] 1.1 In `core/scripts/correction.ts`, add the `control_attribution` record type with bounded
...
- [x] 1.2 Add a pure `deriveAttributionId(...)` helper (stable hash of the identifying fields) so
...
- [x] 1.3 Add `emitControlAttribution(...)` that sanitizes `note`/`evidence_ref.id` through the
...
- [x] 2.1 Add a `correction attribute` sub-command dispatch in `core/scripts/pipeline.ts` and a
      ...
      `evidenceRef`, `note`).
- [x] 2.2 Prove the authority boundary: the command is never wired to advance/unblock/override/
      merge/deploy, and no issue-close or PR-merge path writes an attribution.

## 3. Scoreboard recurrence aggregation

- [x] 3.1 In `core/scripts/scoreboard.ts`, during the existing run scan, read `correction_event`
...
- [x] 3.2 Read `.agent-pipeline/control-attributions.jsonl`, join attributions to classes by
      `correction_key`, and compute `time-to-control` per attributed class.
- [x] 3.3 Evaluate post-control recurrence over eligible exposure (runs after `effective_at` that
...
- [x] 3.4 Resolve supersession chains (latest non-`rejected` implemented boundary), surface
      superseded/rolled-back controls, and keep all wording temporal (no causal claim).

## 4. Grouping, trends, and top-recurring output

- [x] 4.1 Add the `--corrections-by <dimension>` flag (`repo`, `stage`, `harness`, `model`,
...
- [x] 4.2 Carry per-period recurrence totals inside the existing `--bucket` series; render a
      top-still-recurring-classes list with sanitized evidence pointers.
- [x] 4.3 Emit the recurrence sections additively in human, `--json`, and `--html` output without
      changing existing keys or `schema_version` semantics.

## 5. Diagnostics, read-only, and tolerance

- [x] 5.1 Surface malformed/partial/old-schema `correction_event` and `control_attribution`
...
- [x] 5.2 Assert the scoreboard invokes no GitHub command and writes nothing under
      `.agent-pipeline/` (including the attribution ledger).

## 6. Regression fixtures and CI

- [x] 6.1 Fixtures proving: falling recurrence after a `deterministic-gate`/`eval` control;
...
- [x] 6.2 Prove each test bites (fails without the change); run `npm run ci`, regenerate and commit
      the `plugin/` mirror.
