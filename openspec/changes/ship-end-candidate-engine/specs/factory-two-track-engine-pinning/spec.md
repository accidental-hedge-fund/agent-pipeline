...
Ship-end verbs that score or publish the candidate (`factory-release prepare`, `factory-gate`, `pipeline release`, `release finish`, and composer-invoked tag) SHALL execute the candidate engine track after train-complete. That execution SHALL be labeled track `candidate` in run or ship evidence when those runs record engine identity. It SHALL NOT be recorded as track `pinned` solely because a production pin exists.
