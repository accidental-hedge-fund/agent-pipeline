...
1. **Independent agent plan review** — the secondary/reviewer harness (or configured reviewer) reviews the posted implementation plan and produces plan-review evidence (for example a `## Plan Review` comment with a structured verdict). This is agent evidence, not human approval.
...
- **WHEN** operator-facing documentation describes the `plan-review` stage
...
- **THEN** it SHALL state that revision proceeds using independent agent plan-review feedback only
