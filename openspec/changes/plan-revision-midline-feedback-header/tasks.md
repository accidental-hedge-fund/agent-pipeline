...
- [ ] 1.1 Reconstruct the #622 / #658 plan-revision stdout fixture: preamble glued to
...
- [ ] 1.2 Add the fixture to `core/test/verify-harness-commits.test.ts` (or
...
- [ ] 2.1 In `core/scripts/verify-harness-commits.ts`, normalise mid-line /
...
- [ ] 2.2 Keep fence/bold/case/duplicated-header behaviour and both existing block reason
      strings unchanged for true negatives.
- [ ] 2.3 Unit tests: mid-line header + tagged items → `ok: true`; mid-line header with no
...
- [ ] 3.1 In the shared planning phase path (`core/scripts/stages/planning.ts` /
...
- [ ] 3.2 On repair success, continue the normal post-revision path (revalidate, human-
      feedback ack, post revised plan, advance).
- [ ] 3.3 On budget exhaustion, block without posting the revised plan; terminal disposition
      remains `needs-human` with the existing reason strings where applicable.
- [ ] 3.4 Preserve freeform / OpenSpec paired blocker equivalence for this failure mode.
- [ ] 3.5 Stage-level tests with injected harness fakes: (a) first stdout mid-line but valid
...
- [ ] 4.1 Update `core/scripts/prompts/plan_revision.md` so the acknowledgement-format
      instructions require the header at **line start** (in addition to unfenced + exactly
      once).
- [ ] 4.2 Extend `core/test/prompt-loader.test.ts` drift guard for the line-start wording.
