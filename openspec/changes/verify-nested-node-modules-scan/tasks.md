...
- [ ] 1.1 In `core/scripts/verify-harness-commits.ts`, replace the root-only check
...
- [ ] 1.2 Keep the existing diagnostic shape:
      `Commit <sha> adds a node_modules entry (<path>); node_modules must not be committed`.
- [ ] 1.3 Confirm the scan still runs first on non-empty ranges and still ignores
      delete-only paths (behavior of `gitDiffTreeFiles` / `--diff-filter=d` unchanged).

## 2. Regression tests

- [ ] 2.1 Add a nested monorepo case in `core/test/verify-harness-commits.test.ts`
      (e.g. `apps/web/node_modules/.pnpm/...`) that expects `ok: false` and a reason
      naming the nested path.
- [ ] 2.2 Make the nested case bite: assert (or structure the test so) that the legacy
      root-only leading-component check would not flag the nested path, proving the
      segment-aware check is load-bearing.
- [ ] 2.3 Keep existing #180 root-level cases green (`node_modules`,
      `node_modules/foo`, empty range, delete-only cleanup, diagnostic-before-message).
- [ ] 2.4 Optionally add a non-hit case where `node_modules` is only a substring of a
      path component (e.g. `src/node_modules_backup/x.ts`) if not already covered.
- [ ] 2.5 Run the verify-harness-commits tests and confirm the nested case fails without
      the production fix and passes with it.

## 3. Mirror, CI, and scope guard

- [ ] 3.1 After any `core/` edit, run `node scripts/build.mjs` and include regenerated
      `plugin/` in the same change.
- [ ] 3.2 Confirm salvage pathspec composition is untouched (no drive-by edits to
