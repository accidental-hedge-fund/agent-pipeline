...
Artifacts signed before this change encode a root-inclusive `dependency_closure_sha256`. Ready after this change recomputes a root-exclusive hash. Recovery is an authenticated non-ready preview/apply: when the live body already has a Decisions artifact, the only stale fingerprint is `dependency_closure_sha256`, and the recorded hash equals the legacy root-inclusive closure of an authenticated historical pre-proposal snapshot (GitHub `userContentEdits` previous bodies) plus the current declared-dependency snapshot, preview signs the current root-exclusive hash without calling Implementer or Reviewer, and apply writes that snapshot. The current applied specification core is not that snapshot. A missing or non-matching snapshot fail-closes to the normal refine-spec flow. A real declared-dependency title or body change fails that proof and uses the normal refine-spec flow. Settled nodes and answered handoffs stay. Ready still compares the single current formula. A ready-only dual-formula compare is forbidden.

## Risks / Trade-offs

- **[Risk] Old signed artifacts remain stale after this lands** → Mitigation: `pipeline refine-spec --issue N` then `apply` is the authenticated refresh. When only `dependency_closure_sha256` is stale and the recorded hash equals the legacy root-inclusive closure of an authenticated historical pre-proposal snapshot, preview does not re-grill, so extra handoff answers are not required. GitHub issue body revisions are the durable snapshot source. Missing snapshots fail closed. Ready does not dual-compare formulas.
