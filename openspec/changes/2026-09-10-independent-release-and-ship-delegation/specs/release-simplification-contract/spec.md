...
An interrupted publication SHALL remain anchored to proven C rather than rebinding to later main or rerunning fixtures. `release.yml` SHALL be the single publication and recovery owner: an exact-C push run may be rerun, and a missing run may receive one bounded exact-C recovery dispatch to that same workflow after re-verifying the remote annotated tag. The CLI SHALL NOT implement a second Release creator/editor. Release absence may be concluded only from a status-aware authoritative not-found response for that exact tag; authentication, authorization, network, rate-limit, malformed response, and 5xx failures are unknown and fail closed.
...
- **THEN** release reruns or boundedly dispatches `release.yml` for that exact tag and C
...
- **THEN** the command rejects before any Git mutation or performs all alignment and writes inside the dedicated metadata worktree
...
Immediately before tag creation, release SHALL freshly observe origin/main equals frozen C and revalidate the milestone after FRG. It SHALL create and push only an exact annotated tag object by a non-force ref creation, then re-fetch and verify the remote tag object, peeled C, notes, and origin/main. Before publication, `release.yml` SHALL independently verify that the remote tag is the expected annotated object at C and that origin/main still equals C. If any observation differs or is unknown, publication SHALL fail closed. The workflow SHALL check out and verify the intended tag source, including both root and core package versions, before publication; for post-tag docs it SHALL check out the intended current main source before installing dependencies and generating docs.
