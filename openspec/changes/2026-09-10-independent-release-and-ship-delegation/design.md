...
Bounds come from that remote set across reinvocations. A local `recoveryAttempted` flag is not the bound. The recovery episode is a persist-before-dispatch checkpoint of the one missing-run dispatch so a later process does not treat GitHub lag as a new absence. Dispatch is only for absence with no episode. A failed push or dispatch run is rerun in place; it never authorizes a second dispatch. CLI never creates or edits a GitHub Release.
