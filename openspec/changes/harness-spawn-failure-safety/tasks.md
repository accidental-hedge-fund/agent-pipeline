...
- [ ] 1.1 In `core/scripts/harness.ts`, wrap the `spawnImpl(cmd, args, { … })` call in a
  `try/catch` (narrowly around the construction call only, not the whole Promise executor).
- [ ] 1.2 On catch, compute `duration = (Date.now() - start) / 1000` and `settle`/resolve a
...
- [ ] 1.3 NUL-byte detection: when `err.code === "ERR_INVALID_ARG_VALUE"` and the message
...
- [ ] 1.4 Ensure the `catch` short-circuits before any listeners/timers that assume a live
  `child` are attached (i.e. return/resolve immediately from the catch).

## 2. Confirm downstream blocked path is unchanged

- [ ] 2.1 Verify (read-only) that `invoke()`'s custom-CLI `spawn_error` message wrapping and
  `harnessOutcome()`'s `"spawn_error"` classification still apply to the new result — no code
  change expected.
- [ ] 2.2 Verify (read-only) that `advanceReview`'s `!result.success` branch calls
...
- [ ] 3.1 Injected-seam test: call `runCapped` with `opts.spawnFn` that throws a `TypeError`
...
- [ ] 3.2 Non-NUL synchronous throw: inject a `spawnFn` throwing a generic error; assert a
  resolved `spawn_error` result WITHOUT the NUL marker (generic path).
- [ ] 3.3 Real-NUL end-to-end: call `invoke("<cli>", cwd, promptWithNul)` where `promptWithNul`
...
- [ ] 3.4 Prove the tests bite: temporarily remove the `try/catch` from 1.1, run the suite,
  confirm 3.1/3.3 throw/reject (fail), then restore.

## 4. Mirror + CI

- [ ] 4.1 `node scripts/build.mjs` — regenerate the `plugin/` mirror.
- [ ] 4.2 `npm run ci` green end-to-end from the repo root.
