...
- [ ] 1.1 Author `docs/factory-reliability-gate-runbook.md` (or hosts-skill equivalent) covering:
...
- [ ] 1.2 Define the machine-readable FRG evidence JSON schema (`schema_version`, `version`,
      `run_id`, `pass`, `scenarios[]`, `scoreboard`, `thresholds`) and document field meanings in
      the runbook.
- [ ] 1.3 Map each scenario id (`capacity-blocked-retain`, `resume-mid-flight`,
...
- [ ] 2.1 Add hermetic composition tests for `capacity-blocked-retain` (low max worktrees, blocked
      retain ≥ N, next eligible item must not false-block as needs-human solely for capacity).
- [ ] 2.2 Add hermetic tests for `resume-mid-flight` (supervisor interrupt/resume; no permanent dead
      `pr_opened` strand; live next_action for in-flight items).
- [ ] 2.3 Add hermetic tests for `openspec-multi-change` (shared active-change set / archive pass
      vs residual still-active coherence; foreign or partial archive cases).
- [ ] 2.4 Add hermetic tests for `implement-lockfile-dirt` (uncommitted lockfile after HEAD
      advanced; fold/clean path; no human-block on known lock dirt with 0 attempts).
- [ ] 2.5 Add hermetic tests for `local-docs-parity` (docs/generator failure surfaces before PR open
      or before ready-to-deploy, matching CI parity intent).
- [ ] 2.6 For any required Layer A scenario not yet testable, add an explicit waiver entry naming
      scenario id + tracking issue (no silent gaps); prove each implemented assertion bites.
- [ ] 2.7 Confirm Layer A tests use injected deps only (zero real network/git/subprocess) and run
      under `core` unit suite / `npm test`.

## 3. Live Layer B driver

- [ ] 3.1 Implement FRG driver CLI entrypoint (e.g. `pipeline factory-gate` or
      `pipeline release-check --for <version>`) that resolves target version and documented
      concurrency settings.
- [ ] 3.2 Implement scenario-pack work-list selection or fixture creation (synthetic/scratch or
      labeled reliability pack — not full product milestone).
- [ ] 3.3 Wire driver to start multi-item durable loop via shipped runtime (no second ledger/lock
      namespace).
- [ ] 3.4 Project per-item outcomes and blocker taxonomy scoreboard from durable ledger/events into
      the FRG evidence report; compute overall `pass` against runbook thresholds (K, N, engine-class
      rate, required scenario outcomes).
- [ ] 3.5 Persist immutable evidence under the stable path from the runbook; support `--json`
      machine-readable output; exit non-zero on `pass: false` or incomplete evidence.
- [ ] 3.6 Cover `pr-supersession`, `release-plan-row`, and `empty-depends-on-stack-honesty` in the
      live pack (and hermetic where feasible); surface warn/fail for empty-depends_on OpenSpec
      stacking.

## 4. Release integration

- [ ] 4.1 Extend `pipeline release` to look up FRG evidence for the resolved version and fail closed
      when missing, unparsable, or `pass: false` (additive to existing `npm run ci` gate).
- [ ] 4.2 On successful FRG check, attach/include FRG `run_id` + pass summary on the release PR
      surface (body section or comment).
- [ ] 4.3 Add unit tests for release FRG refusal and attachment seams via injected deps (no real
      network).
- [ ] 4.4 Document the release refuse rule in release/operator docs cross-linking the FRG runbook.
...
- [ ] 5.3 Confirm a subsequent release can reuse the same driver + runbook without a one-off
...
- [ ] 6.1 Regenerate `plugin/` via `node scripts/build.mjs` after any `core/` changes; commit
      mirror with core.
- [ ] 6.2 Run `npm run ci` green (including hermetic FRG pack and `openspec validate --all`).
