...
See `proposal.md` for why. Issue #1201 is a synthetic Factory Reliability
Gate (FRG) pack item (`template_id=clean-openspec`,
`pack_run_id=pack-1399-tugboat-ship-1.39.9`). It is not an engine-dogfood
...
Sibling pack item #1200 (`clean-docs`) uses the same
`pack-1399-tugboat-ship-1.39.9` directory. This change adds
`clean-openspec.json` beside it. It does not replace that sibling file.
...
- One co-located unit test that asserts `release_version === "1.39.9"`.
- Keep this the only active OpenSpec change for #1201.
...
**Choice:** The fixture uses `release_version` with value `1.39.9`.
...
**Choice:** Add `core/test/frg-pack-1399-clean-openspec.test.ts`. It reads
...
- [Sibling #1200 already owns the pack-run directory] → Add only
  `clean-openspec.json`. Do not delete or rewrite `clean-docs.json`.

## Migration Plan

No production migration. Pre-merge archives
`frg-pack-1399-clean-openspec` into
`openspec/specs/frg-pack-1399-clean-openspec/spec.md`. Rollback is revert
