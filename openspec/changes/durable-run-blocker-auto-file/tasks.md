...
- [ ] 1.1 Add a read-only projection over the durable-loop store
...
- [ ] 2.3 Derive a deterministic *suggested* milestone from the blocker class
      (advisory only) and surface it in the cluster entry.
- [ ] 2.4 Extend `proposedTitle` so a `durable-run-blocker` cluster's title
      identity is `(class, fingerprint)`, never free-text prose.

## 3. Qualification predicate

- [ ] 3.1 Qualify a cluster when a terminal stop is attributable to it **or** it
...
- [ ] 3.2 Ensure the shared dedup/rate-cap in `autoFileClusterCategory` runs over
      the already-qualified set unchanged.

## 4. Report + auto-file wiring

- [ ] 4.1 Render `durable-run-blocker` clusters in the dry-run report and JSON
      output: fingerprint, runs affected, theme, and suggested milestone.
- [ ] 4.2 Add a `DURABLE_RUN_BLOCKER_AUTO_FILE_CATEGORY` (`AutoFileCategory`) with
...
- [ ] 4.3 Add an `autoFileDurableRunBlockers` entry point reusing
      `autoFileClusterCategory` (dedup, rate cap, sanitization, cross-host
      reconciliation unchanged).

## 5. Config + trigger

- [ ] 5.1 Add a `durable_runs` config block (`auto_file`,
...
- [ ] 5.2 Trigger `autoFileDurableRunBlockers` at durable-run terminal stop /
...
- [ ] 6.1 Terminal-stop cluster files (single run with a terminal
      `LoopStopRecord`).
- [ ] 6.2 Repeated `(class, fingerprint)` across ≥ 2 runs files.
- [ ] 6.3 Single non-terminal occurrence does not file and is not reported as
      qualifying.
- [ ] 6.4 A qualifying cluster whose title matches an open issue is not re-filed
      (dedup).
- [ ] 6.5 Rate cap and sanitization (a secret in evidence is redacted in the
      filed body) hold for this source.
- [ ] 6.6 Inert-by-default: with `durable_runs.auto_file` absent/`false`, no
      issue is created and no `gh` call is made; run artifacts are unchanged.
- [ ] 6.7 Report lists fingerprint, runs, theme, and suggested milestone; the
      filed issue carries no milestone. Prove each test bites (fails without the
      change).

## 7. Mirror + gate

- [ ] 7.1 Regenerate the `plugin/` mirror (`node scripts/build.mjs`) and commit it
      in the same change.
- [ ] 7.2 `npm run ci` green (core tests, mirror check, install smoke,
