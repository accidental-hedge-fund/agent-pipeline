...
- [Risk: Pre-merge guard produces false positives on valid "spec didn't need to change" fixes] The three-condition heuristic (impl changed + spec unchanged + reviewer flagged divergence) reduces false positives to cases where the reviewer was wrong. The implemented escape is to bring the spec delta into agreement: any commit that touches the change's `specs/**` clears the guard. Even when the divergence finding is a false positive, the correct resolution is to update the delta so the living spec states the actual behavior — strictly better than silently archiving an unverified delta. The block message documents this. (We do NOT key the escape off `--override`: that mechanism governs review-routing severity, not the archive layer, so it would not clear this guard.)
