...
- [ ] 1.1 For each of the 12 active change ids, record the merged PR (if any) that shipped it:
...
- [ ] 1.2 For each change, diff its `specs/<capability>/spec.md` deltas against
...
- [ ] 1.3 Write the disposition table (id → classification → archive command → evidence) into
      the implementation PR body.

## 2. Build the guard (before the cleanup, so it proves itself on the mess)

- [ ] 2.1 Add the active-change scan to `scripts/ci-openspec.mjs`: list `openspec/changes/*`
      excluding `archive`, run after `openspec validate --all`, report both failures.
- [ ] 2.2 Add mode resolution: `OPENSPEC_HYGIENE_MODE` → GitHub Actions env
...
- [ ] 2.3 Add allowlist parsing for `openspec/active-allowlist.txt` (one id per line, ignore
      blanks and `#` comments; missing/empty ⇒ strict); error on entries naming a
      non-existent change.
- [ ] 2.4 Write the failure message: every offending id on its own line plus the cleanup path
      (pre-merge archiving, or `openspec archive <id>`).
- [ ] 2.5 Create `openspec/active-allowlist.txt` containing only header comments (no entries).

## 3. Tests

- [ ] 3.1 Extend `scripts/ci-openspec.test.mjs` with fixture repos under a temp dir, driven via
      `CI_OPENSPEC_ROOT` and `OPENSPEC_HYGIENE_MODE` — no network, git, or `gh` calls.
- [ ] 3.2 Cases: unallowlisted active change in default-branch mode fails and names the id;
...
- [ ] 3.3 Prove the guard tests bite — confirm they fail against the pre-change
      `ci-openspec.mjs`.

## 4. Cleanup

- [ ] 4.1 Archive each *already in living specs* change with `openspec archive <id> --skip-specs`.
- [ ] 4.2 Archive each *shipped but missing from living specs* change with
      `openspec archive <id> --yes`, then review the resulting `openspec/specs/` diff.
- [ ] 4.3 Archive each *never shipped / superseded* change, adding
...
- [ ] 4.4 Run `openspec validate --all` after **each** archive step so a bad spec merge is
      attributed to the change that caused it.
- [ ] 4.5 Confirm `openspec list` reports zero active changes other than this change itself.

## 5. Verify

- [ ] 5.1 `OPENSPEC_HYGIENE_MODE=default-branch npm run ci:openspec` fails against this
...
- [ ] 5.3 Confirm no `core/` files changed, so no `plugin/` mirror regeneration is required
