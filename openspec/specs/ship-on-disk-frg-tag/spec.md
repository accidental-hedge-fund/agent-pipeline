# ship-on-disk-frg-tag Specification
...
#### Scenario: Remote-only correct tag is exists without on-disk HMAC

- **WHEN** local `refs/tags/v1.39.5` is missing
- **AND** origin has annotated `refs/tags/v1.39.5` on the merge commit
- **AND** on-disk HMAC `latest.json` is absent
- **THEN** `release ensure-tag` SHALL succeed as exists
- **AND** SHALL NOT require on-disk `latest.json`
...
#### Scenario: Unauthenticated factory_release_binding overlay prevents tag create
...
The helper SHALL tag the peeled merge commit. It SHALL NOT retarget the tag to the packed candidate when those SHAs differ because the release pull request added commits. It SHALL NOT rewrite `latest.json` so `candidate_git_sha` equals the merge commit. A `candidate_git_sha` that equals neither this ship's packed candidate nor a recorded packed-candidate identity SHALL fail closed. HMAC `candidate_git_sha` SHALL be taken from the same HMAC-validated snapshot used for release-eligibility. An unauthenticated `factory_release_binding` overlay SHALL fail closed. A present but invalid binding SHALL NOT fall back to `pack_provenance`.
