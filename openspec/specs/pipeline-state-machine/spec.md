...
Loop-isolated merge surfaces exist separately. A direct operator or external supervisor under operator authority MAY invoke `pipeline merge <pr>` or `pipeline merge-queue ... --apply` under those commands' gates. Those surfaces SHALL remain outside stage dispatch and repository configuration. Their use does not add a merge stage and does not let ordinary `advance` or `loop` merge.
...
- **WHEN** a direct operator or external supervisor invokes `pipeline merge <pr>` after an issue is at `pipeline:ready-to-deploy`
