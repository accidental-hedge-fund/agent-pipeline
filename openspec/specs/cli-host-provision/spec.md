# cli-host-provision Specification

## Purpose
Install the pipeline CLI for Claude the same way as Codex and Grok: atomically stage launcher, core tree, and SKILL; best-effort prewarm dependencies; and preserve first-run self-heal. Do not vendor engine source into `plugin/` and do not emit a `/pipeline:*` command pack.
## Requirements
### Requirement: Claude install SHALL provision the pipeline CLI

`install --host claude` (not dry-run) SHALL atomically stage the pipeline CLI under the resolved Claude skill install: launcher, whitelisted `core/` tree, and Claude host SKILL overlay. After the atomic swap, the installer SHALL best-effort prewarm runtime dependencies with `npm ci` at `core/node_modules`. Missing npm or failed prewarm SHALL emit a named warning and SHALL NOT discard the completed install. A failed prewarm SHALL remove any partial `core/node_modules` so the launcher performs its documented first-run self-heal before a dependency-requiring verb dispatches. If that self-heal also fails, the invocation SHALL fail closed with remediation. Claude SHALL get the engine the same way as Codex and Grok. The SKILL SHALL instruct the agent to exec `pipeline <verb>`. Short SKILL prose is owned by #1049; this requirement is satisfied by pointing install at the current host SKILL overlay. Dry-run SHALL NOT write the skill tree.

#### Scenario: Claude install atomically writes launcher core and SKILL
...
- **WHEN** staging and atomic swap succeed but install-time npm is absent or `npm ci` exits non-zero after creating a partial `core/node_modules`
...
- **AND** partial `core/node_modules` SHALL be absent
