# loop-precondition-stage-gate Specification
...
carrying `pipeline:needs-spec`, or carrying no `pipeline:*` label at all — the supervisor SHALL exclude that item from the
...
it with no real network, git, or subprocess call. An item observed at `pipeline:ready` is still subject to the shared issue-implementation-readiness gate at dispatch when that gate is enabled; passing this precondition is not a bypass of that gate.
...
#### Scenario: A needs-spec item is excluded, not dispatched

- **WHEN** an eligible work-list item is observed carrying `pipeline:needs-spec`
- **THEN** the supervisor SHALL NOT dispatch that item this cycle
- **AND** it SHALL record a non-fatal `precondition` exclusion naming the item, the required
  stage `pipeline:ready`, and the observed stage `pipeline:needs-spec`
- **AND** the exclusion SHALL NOT consume any recovery budget and SHALL NOT record a run stop
...
#### Scenario: A ready item alongside a needs-spec item still advances

- **WHEN** a run's frontier contains one item at `pipeline:ready` and one item at
  `pipeline:needs-spec`
- **THEN** the supervisor SHALL dispatch the `pipeline:ready` item
- **AND** SHALL exclude the `pipeline:needs-spec` item with a `precondition` rationale
...
an item and the item is left still at a pre-pipeline stage (`pipeline:backlog`, `pipeline:needs-spec`, or no
...
A dispatch that starts at `pipeline:ready` and moves to `pipeline:needs-spec` because the issue-implementation-readiness gate returned `needs_spec` SHALL be a structured admission rejection, not `workflow-engine-defect` and not a `run_fatal` stop.
...
#### Scenario: A zero-transition needs-spec dispatch is not run-fatal
...
#### Scenario: A needs_spec admission rejection is not an engine defect

- **WHEN** an item is dispatched at `pipeline:ready` and the issue-readiness gate moves it to `pipeline:needs-spec`
- **THEN** the outcome SHALL be recorded as a structured admission rejection
- **AND** it SHALL NOT be classified `workflow-engine-defect`
- **AND** it SHALL NOT record a `run_fatal` stop
