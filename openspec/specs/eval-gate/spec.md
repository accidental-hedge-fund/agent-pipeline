...
After each eval run (pass or fail, gate or advisory), the stage SHALL post a `## Eval Gate` comment on the issue containing: mode, outcome (PASS/FAIL), elapsed time, and up to the first 2000 characters of the combined stdout/stderr output.
...
- **AND** the comment SHALL include elapsed time and a stdout excerpt (≤2000 chars)

#### Scenario: comment posted on fail
- **WHEN** the eval command exits non-zero
- **THEN** a comment beginning with `## Eval Gate` SHALL be posted on the issue
- **AND** the comment SHALL state the outcome as FAIL
- **AND** the comment SHALL include elapsed time and a stdout/stderr excerpt (≤2000 chars)
