...
Multi-change maintainability experiments SHALL activate a design-dossier / human-attestation treatment variant only when `#575` / `pre_code_attestation` controls are configured and the risk policy fires for the fixture. Absence of `#575` configuration SHALL NOT prevent bare-versus-pipeline execution. When activated, the treatment SHALL reuse the same benchmark contract and deterministic verifiers as other treatments.
