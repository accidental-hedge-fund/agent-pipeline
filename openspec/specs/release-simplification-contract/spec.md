...
- **AND** a missing-run dispatch is recorded in a durable recovery episode in a host-local location shared by all worktrees of the repository before the workflow run, and a later invocation from any worktree of that repository with that episode still absent waits instead of dispatching again
...
#### Scenario: Publisher recovery episode is shared across sibling worktrees

- **WHEN** a missing-run dispatch is recorded from one worktree of a repository
- **AND** a later invocation from a sibling worktree of that repository still observes no exact-identity run
- **THEN** recovery waits on the existing episode instead of dispatching again
