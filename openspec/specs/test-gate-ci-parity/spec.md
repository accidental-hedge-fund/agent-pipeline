...
- **THEN** it SHALL invoke `node scripts/build.mjs --check` (directly or transitively) so that a stale generated host SKILL causes the script to exit non-zero

#### Scenario: stale generated packaging artifact is caught at the test gate, not at CI

- **WHEN** a pipeline run leaves a generated host SKILL stale
...
- **WHEN** `plugin/` is absent
- **AND** the four generated host SKILLs match
- **AND** the test gate runs `npm run ci`
- **THEN** `node scripts/build.mjs --check` SHALL NOT fail solely because `plugin/` is absent
