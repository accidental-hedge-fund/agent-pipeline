...
2. **`npm run ci` must pass before a change is done.** It runs: `ci:core`
   ...
   `openspec/` directory is present) → and, when `scripts/generate-docs.mjs` is
   present, **docs freshness** (`docs:check` / `generate-docs --check`). Green
   unit tests alone do not mask stale generated docs. Treat a red `ci` as not-done.
...
When the docs generator is present, the same full gate includes docs freshness
so stale generated docs fail before PR open — the same way a stale `plugin/`
mirror fails `build.mjs --check`.
