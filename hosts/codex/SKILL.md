...
$pipeline:merge-queue --milestone <m> --apply --repair  typed conflict/CI holds; optional surgical repair then re-gate
...
- Auto-merge PRs autonomously — the advance loop never merges and there is no `auto_merge` config key. The human-invoked `$pipeline merge <pr>` command is the controlled, explicit surface for merging after `pipeline:ready-to-deploy`; it is never called by the advance loop. `$pipeline:merge-queue` is human-gated (default dry-run; `--apply` uses the same merge surface; optional `--repair` for surgical conflict/CI holds with re-gate) and is never called by advance.
