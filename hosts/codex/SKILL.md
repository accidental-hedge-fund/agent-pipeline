...
The primary invocation is the single `$pipeline` skill entry. It forwards its
argument string to the CLI operations cataloged below; there are no per-verb
skill-menu entries.
...
When the ledger carries `stop.reason = run_fatal`, resume re-drives valid
outstanding items at the same run id after a fresh reconcile, or refuses with
the recorded fatal time/theme and `pipeline loop --resume <run-id> --audit` /
`pipeline loop --new-run`; it never silently reports `resumed: true` with
`dispatched: 0`.
...
the user (CLI exits 0 on terminal by default). Prefer the shared
**events.jsonl material filter** for host notify; unfiltered events remain
...
```bash
node ~/.codex/skills/pipeline/scripts/pipeline.mjs logs <train-run-id> --events --follow \
  | node ~/.codex/skills/pipeline/scripts/material-filter.mjs
```
...
```bash
node ~/.codex/skills/pipeline/scripts/pipeline.mjs loop logs <loop-run-id> --events --follow \
  | node ~/.codex/skills/pipeline/scripts/material-filter.mjs
```

4. Notify via the host map (Codex: concise chat/status; never Claude `PushNotification`) on material train lines. **Re-arm** material follow after wait cancel until train `run_complete`.
