...
- `opencode` CLI on PATH — default implementer for this host profile
- A registered reviewer harness on PATH (default profile: `claude`; override via
  `harnesses.reviewer` in `.github/pipeline.yml`)
- Node 24+
- Authenticated provider credentials for the OpenCode and reviewer harnesses
...
```bash
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs <N> --status
```
...
```bash
cd <repo_dir>
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs single <N>
```
...
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop logs <run-id> --events --follow \
  | node ~/.config/opencode/skills/pipeline/scripts/material-filter.mjs
...
```bash
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop --resume <run-id> --audit
```
...
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop --milestone <name> \
  >"$LOOP_OUT" 2>"$LOOP_ERR" &
...
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop logs <run_id> --events --follow \
  | node ~/.config/opencode/skills/pipeline/scripts/material-filter.mjs
...
```bash
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs logs <advance-run-id> --events --follow \
  | node ~/.config/opencode/skills/pipeline/scripts/material-filter.mjs
```
...
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop --resume <run-id> --audit --follow
```
...
```bash
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs logs <train-run-id> --events --follow \
  | node ~/.config/opencode/skills/pipeline/scripts/material-filter.mjs
```
...
```bash
node ~/.config/opencode/skills/pipeline/scripts/pipeline.mjs loop logs <loop-run-id> --events --follow \
  | node ~/.config/opencode/skills/pipeline/scripts/material-filter.mjs
```
