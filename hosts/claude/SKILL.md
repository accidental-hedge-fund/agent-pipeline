...
pipeline train --milestone <m> [--merge] [--json] [--dry-run] | pipeline train --issues <n,n> [--merge] [--json] [--dry-run] Operator-authorized integrate train: base-eligible frontiers advance via one loop wave each (recovery inside the wave); optionally serial-merge with base containment; independent R2D siblings may merge while a peer is parked (never called by the advance loop)
