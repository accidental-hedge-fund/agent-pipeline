...
pipeline ship --milestone vX.Y.Z [--json] | pipeline ship status --milestone vX.Y.Z [--json] Ship-final-delegation: train --merge, then exactly one complete-release call. Direct-release and this tail complete the same SemVer contract. Neither path deploys, promotes, or installs. Operator product is pipeline ship --milestone vX.Y.Z; no grant file required.
pipeline release <version> [--theme "..."] [--dry-run|--json] [--no-edit] | pipeline release prepare <version> [--json] | pipeline release finish <pr> [--json] Direct-release: independently complete the SemVer contract for an exact candidate (milestone, metadata, exact-candidate FRG, annotated tag C, publication). Explicit prepare is the bounded metadata helper and never tags. release finish is a metadata-PR merge helper, not the live complete-release command. Neither path deploys, promotes, or installs
