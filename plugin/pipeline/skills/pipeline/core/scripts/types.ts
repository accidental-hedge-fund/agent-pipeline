...
export const BLOCKER_RECIPES: Record<BlockerKind, string> = {
  "needs-human":
    ...
    "`pipeline:blocked` label, and re-run `$pipeline {{N}}`. Or record an " +
  ...
  "test-gate-exhausted":
    ...
    "the fix, remove the `pipeline:blocked` label, then re-run `$pipeline {{N}}`.",
  "no-commits":
  ...
  "openspec-invalid":
    ...
    "`pipeline:blocked` label, then re-run `$pipeline {{N}}`.",
  "openspec-stale-delta":
  ...
  "merge-conflict":
    ...
    "remove the `pipeline:blocked` label, then re-run `$pipeline {{N}}`.",
  "worktree-missing":
    "The worktree for this issue no longer exists, so fixes can't be applied. " +
    "Remove the `pipeline:blocked` label and re-run `$pipeline {{N}}` to " +
