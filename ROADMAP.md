...
| **v1.15.0** | minor | Factory reliability: fix-round convergence, wedge-proof timeouts, de-flaked gates, single-operator human-input gate | #391, #398, #403, #390, #393, #387 | Behavioral hardening + additive identity/verification surfaces, no config or output-schema breaks: fix rounds recognize dispositioned/non-reproducing findings via fingerprint-qualified identities instead of dead-ending (#391/#404), runCapped resolves unconditionally past its timeout with a post-SIGKILL failsafe (#398/#405), the event-sink stdin EPIPE/close race that flaked every test gate is fixed (#403/#406), the human-input gate stops counting the pipeline’s own bodyHash-verified review output in single-operator repos (#390/#407), reviewer spawn survives NUL bytes in prompt payloads as a blocked state instead of a process crash (#393/#408), and fix/auto-fix rounds rebuild declared generated artifacts before committing (#387/#409). |
