...
| **v1.14.1** | patch | Gate/CLI reliability: test-gate capture resilience + wrapper --profile fix | #384, #383 | Two merged bug fixes, no config or schema changes: test-gate outcome derives solely from the command exit code with sink-write failures isolated as diagnostics and tail-biased excerpts (#384/#397), and the host wrapper no longer injects `--profile` into commands that reject it — `refine-spec`, `scoreboard`, and `release` work through the installed wrapper (#383/#394). |
