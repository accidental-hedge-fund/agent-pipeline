...
| **v1.15.2** | patch | Reviewer model/effort passthrough for codex + gitignored-artifact commit guard | #441, #445 | Repo config could name a reviewer model/effort but a codex reviewer ignored both (#441 — now honored with correct auto-vs-explicit sentinel handling). Harness commit steps silently dropped gitignored artifacts the change depended on, surfacing only as mysterious downstream CI failures (#445 — now detected and warned loudly in stage output and run evidence). Additive; existing flows unchanged. |
