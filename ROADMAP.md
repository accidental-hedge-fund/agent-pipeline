...
| **v1.14.0** | minor | Convergence & evidence: post-fix re-review correctness, eval-gate fix routing, durable evidence, crash recovery | #373, #371, #377, #372, #382 | Behavioral hardening + additive evidence surfaces, no config or output-schema breaks: eval-gate failure output tail-biased (#373), pre-merge auto-fix re-review anchored to the true post-fix head with live-ref confirmation (#371), per-stage timing table + append-only issue evidence history in PR comments (#377), eval-gate failures route through a fix round with eval output as context (#372), crash-orphaned `implementing` recovers via stranded-attempt restart (#382). |
