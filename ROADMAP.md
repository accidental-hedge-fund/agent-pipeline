...
| **v1.15.2** | patch | Reviewer CLI model/effort passthrough: honor configured `models.review` + `effort.review` for the codex reviewer | #441 | Repo config could name a reviewer model/effort, but a codex reviewer ignored both (config warning only). Now honored with correct auto-vs-explicit sentinel handling. Additive; existing flows unchanged. |
