...
| **v1.5.0** | minor | Pipeline Desk desktop contracts + carried autonomy | #153, #154, #155, #156, #161, #23, #149 | Machine-facing launch, status, event, log, config-validation, and run-artifact-convention contracts so Pipeline Desk can supervise runs without scraping terminal prose. Plus carried-forward **#23** (graduated-autonomy approval checkpoints — still parked on the checkpoint forge-resistance security property, PR #194 open) and **#149** (bounded auto-loop, depends on #23). Keeps the current skill structure and human `/pipeline` / `$pipeline` flows intact. |
