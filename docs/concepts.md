...
Train follow: parse `train_run_handoff` for `run_id`, then `pipeline logs <train-run-id> --events --follow` through the shared material filter. When `train_loop_linked` publishes a loop id, dual-follow that loop stream. That event is published from the child `onRunReady` handoff (exact loop run id and absolute events path) while the child can still be live — do not wait for the wave to return. Train omits `train_loop_linked` when the events path is missing, empty, or not absolute. A later handoff that disagrees on run id or path keeps the first link and sets `events_coverage` to `degraded`. Same-clock trains receive distinct `train-<timestamp>` directories (suffix `-2`…`-8` on `EEXIST`); exhausted allocation omits `run_id` and sets `train_status.events_coverage` to `degraded` or `unknown`. `train_merge_proven` includes `proof_disposition` `newly-merged` or `already-contained`.
