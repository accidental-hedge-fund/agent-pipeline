...
| `needs-human` / blocked — **residual review park at current HEAD** (after deterministic resume) | Report the park as a RecoverySupervisor observation. Train does **not** auto-invoke recover-parked. Operators and external supervisors may still run **`pipeline recover-parked <N>`** as a CLI. Deterministic recover (scratch unlink, unpublished-commit publish, stale-block resume) already runs **inside the advance wave** before requiring a linked open PR. recover-parked reflows only stale/DNR/below-high via audited override; **never** auto-overrides HIGH/CRITICAL/security/authority. Pre-PR engine-defect parks without a PR re-enter advance. If still parked → hold the item (independent siblings continue) and notify human. Do **not** invent `pipeline override` or drop `blocked` from the host. |
