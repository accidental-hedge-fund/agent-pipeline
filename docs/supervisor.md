...
| `needs-human` / blocked — **residual review park at current HEAD** (after deterministic resume) | Call **`pipeline recover-parked <N>` once** (train does this automatically). Deterministic recover (scratch unlink, unpublished-commit publish, stale-block resume) runs **before** requiring a linked open PR. It reflows only stale/DNR/below-high via audited override; **never** auto-overrides HIGH/CRITICAL/security/authority. Pre-PR engine-defect parks without a PR re-enter advance. If still parked → STOP + notify human. Do **not** invent `pipeline override` or drop `blocked` from the host. |
