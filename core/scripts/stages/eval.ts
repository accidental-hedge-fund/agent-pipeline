    ...
    afterRound: async (ctx) => {
      ...
      if (!fixRes.success) {
        ...
        const reason = preflightReason
          ? `Fix harness (${harness}) failed (${preflightReason}) on eval-gate fix round ${attempt}.`
          : fixRes.background_wait
            ? `Fix harness (${harness}) missed delivery or foreground-join on eval-gate fix round ${attempt} (harness-background-wait).` +
...
export async function advanceEval(
  ...
  if (!wt) {
    ...
    if (remat.result === "fail" || !remat.worktree) {
      ...
      const reason =
        remat.result === "fail"
          ? `eval-gate: no worktree found and rematerialize failed (${blockerKind}): ${remat.reason}`
          : `eval-gate: no worktree found and rematerialize returned ${remat.result} without a worktree: ${remat.reason}`;
