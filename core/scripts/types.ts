...
export const BLOCKER_RECIPES: Record<BlockerKind, string> = {
  ...
  "review-findings":
    ...
    "`blocked` label, and re-run `$pipeline {{N}}`.",
  "test-gate-exhausted":
...
export interface ReviewFinding {
  ...
  // re-raises, as blocking, a specific finding the prior-round digest marks
  // settled (resolved-by-fix or overridden): names the prior round and explains why a
  // genuinely new resolution — not a reversal — is warranted. Absent, empty, or
  // whitespace-only on a matched settled finding demotes it to advisory with
