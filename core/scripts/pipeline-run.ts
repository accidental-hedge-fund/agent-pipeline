...
export function buildAutoLoopExhaustedComment(
  ...
  return reviewStage.attestPipelineComment(
    ...
    [
      ...
      "",
      `The issue remains blocked at \`${stage}\` with its mechanical blocker kind preserved.`,
      "This handoff is a Cooling observation. RecoverySupervisor retains ownership.",
      "It is not human authority. To resume:",
...
export function createDeliveryStageEvidenceObserver(
  ...
  return async (phase) => {
    ...
    if (deps.expectedPrHeadSha != null) {
      ...
      const localSha = normalizeCandidateSha(candidateSha);
      if (localSha && localSha !== livePrSha) {
        paths = [];
      }
      candidateSha = livePrSha;
    }
