...
test("ARTIFACT_CONTRACT: contains exactly runs/, roadmap/, history/, evals/, control-attributions.jsonl, and product-fault-reports.jsonl with non-empty comments", () => {
  const names = ARTIFACT_CONTRACT.map((e) => e.name);
  assert.deepEqual(names, [
    "runs", "roadmap", "history", "evals", "control-attributions.jsonl", "product-fault-reports.jsonl",
  ]);
