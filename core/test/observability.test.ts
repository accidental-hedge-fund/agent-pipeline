...
import {
  ...
  nativeSessionId, observabilityEnabled, type ObservabilityDeps,
...
function fixture(enabled = true) {
  ...
  const deps: ObservabilityDeps = {
    home: "/host", env: enabled ? { AGENT_OBSERVABILITY_ENABLED: "1" } : {},
...
test("absent config performs no writes; explicit off overrides installed config", async () => {
  const { deps, files } = fixture(false);
  await enqueueAccountingObservation(runDir, record(), deps);
  assert.equal(files.size, 1);
  files.set("/host/.config/agent-observability/config.json", "{}");
  assert.equal(await observabilityEnabled(deps), true);
  deps.env.AGENT_OBSERVABILITY_ENABLED = "0";
  assert.equal(await observabilityEnabled(deps), false);
});
...
test("accounting output is metadata-only, preserves unknowns, and includes qualified work item", async () => {
  ...
  await enqueueAccountingObservation(runDir, r, deps);
...
test("inbox is bounded and write errors never fail the stage", async () => {
  ...
  await assert.doesNotReject(enqueueAccountingObservation(runDir, record(), deps));
...
test("pipeline identity reaches children without papercuts and marker precedes streamed session mapping", async () => {
  ...
  const marker = "/host/.local/state/agent-observability/context/active--invocation-1.json";
  ...
  const context = JSON.parse(files.get("/host/.local/state/agent-observability/context/codex--native-123.json")!);
...
test("unresolved native session leaves a completed marker; failed sidecar never releases deferral", async () => {
  ...
  const observation = (await beginInvocationObservation(input, deps))!;
  await observation.finish("arbitrary text", "2026-09-06T00:01:00Z");
  const marker = JSON.parse(files.get("/host/.local/state/agent-observability/context/active--invocation-1.json")!);
...
test("run-store sink gets final sanitized accounting and failure does not propagate", async () => {
  ...
  const deps = {
    appendFile: async () => {},
    accountingSink: async (_dir: string, r: object) => { events.push(r); },
  } as unknown as RunStoreDeps;
  await emitStageAccounting(runDir, Object.assign(record(), { secret_prompt: "PRIVATE" }), deps);
