...
test("selectIssues: pipeline:needs-spec is not autonomous-eligible", () => {
  const candidates = [
    makeIssue(1, ["pipeline:needs-spec"]),
    makeIssue(2, ["pipeline:ready"]),
    makeIssue(3, ["pipeline:needs-spec"]),
    makeIssue(4, ["pipeline:ready"]),
  ];
  const result = selectIssues(candidates, {}, 10);
  assert.deepEqual(result.map((i) => i.number), [2, 4]);
});

test("runQueue: needs-spec mix only dispatches ready issues", async () => {
  const launched: number[] = [];
  const deps = makeDeps({
    listEligibleIssues: async () => [
      makeIssue(10, ["pipeline:needs-spec"]),
      makeIssue(11, ["pipeline:ready"]),
      makeIssue(12, ["pipeline:needs-spec"]),
    ],
    runPipeline: async (n) => {
      launched.push(n);
      return { issueNumber: n, finalState: "ready-to-deploy", costUsd: 0.1, durationMs: 50 };
  ...
  assert.deepEqual(launched, [11]);
});

test("runQueue: needs_spec on one issue does not abort an independent sibling", async () => {
  const launched: number[] = [];
  const deps = makeDeps({
    listEligibleIssues: async () => [makeIssue(21), makeIssue(22)],
    runPipeline: async (n) => {
      launched.push(n);
      if (n === 21) {
        return {
          issueNumber: n,
          finalState: "needs-spec",
          costUsd: 0.01,
          durationMs: 40,
          error: "issue-readiness: needs_spec: missing observable acceptance criteria",
        };
      }
      return { issueNumber: n, finalState: "ready-to-deploy", costUsd: 0.1, durationMs: 50 };
    },
  });
  await runQueue(makeOpts({ concurrency: 1, maxFailureRate: 0.01 }), deps);
  assert.deepEqual(launched, [21, 22]);
  const json = [...deps.written.values()][0];
  assert.ok(json.includes("needs-spec"));
  assert.ok(json.includes("needs_spec") || json.includes("21"));
});
