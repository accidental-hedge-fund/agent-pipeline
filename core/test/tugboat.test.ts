...
type TugboatSpawnEnvOpts = {
  /** Keep parent TUGBOAT_SKIP_TRAIN / TUGBOAT_CANDIDATE_COMPOSER (v1.39.8 CI bite). */
  inheritParentSkipTrain?: boolean;
  parentEnv?: NodeJS.ProcessEnv;
};

/** Parent env as in a live Tugboat `pipeline release` child (v1.39.8). */
function tugboatReleaseCiParentEnv(): NodeJS.ProcessEnv {
...
function skipTrainProofKind(
  runDir: string,
): "complete" | "train.json" | "stderr" | "state" | null {
  const complete = path.join(runDir, "train.complete.json");
  if (fs.existsSync(complete) && fs.statSync(complete).size > 0) return "complete";
  const trainJson = path.join(runDir, "train.json");
  if (fs.existsSync(trainJson) && fs.statSync(trainJson).size > 0) return "train.json";
  const stderr = path.join(runDir, "train.stderr");
  if (
    fs.existsSync(stderr) &&
    /has no open issues/i.test(fs.readFileSync(stderr, "utf8"))
  ) {
    return "stderr";
  }
  ...
  if (fs.existsSync(state)) {
    ...
    if (/"status"\s*:\s*"ok"/i.test(body) && /no open issues/i.test(body)) {
      return "state";
    }
  }
  return null;
...
function assertSkipTrainProof(runDir: string): void {
  assert.notEqual(
    skipTrainProofKind(runDir),
    null,
    `missing skip-train proof in ${runDir}`,
  );
}
...
test("tugboat train stage-watch argv matches bundled ship-stage-watch.sh (#1184)", () => {
  ...
  assert.match(attachFn, /extract_latest_loop_run_handoff_events/);
  assert.match(attachFn, /reap_stage_watch_pid/);
  assert.doesNotMatch(
    attachFn,
  ...
  const lockIdx = shipOne.indexOf("try_acquire_ship_lock");
  const reapIdx = shipOne.indexOf("reap_stage_watch_pid");
  const attachIdx = shipOne.indexOf("attach_train_stage_watch");
  assert.ok(lockIdx >= 0 && reapIdx > lockIdx, "leftover pid-file reap must run after lock acquire");
  assert.ok(attachIdx > reapIdx, "leftover pid-file reap must run before train watch attach");
  assert.doesNotMatch(shipOne, /pkill\s+.*ship-stage-watch/);
});
...
test("tugboat default SHIP_STAGE_WATCH_BIN is the repo sibling (#1184)", () => {
  ...
  assert.match(startFn, /BUZZ_CREDENTIALS_FILE="\$\{BUZZ_CREDENTIALS_FILE:-\}"/);
  assert.match(startFn, /BUZZ_RELAY_URL="\$\{BUZZ_RELAY_URL:-\}"/);
  assert.match(startFn, /BUZZ_CHANNEL="\$\{BUZZ_CHANNEL:-\}"/);
  assert.match(startFn, /material filter missing/);
  assert.doesNotMatch(
    body,
    /source\s+[^\n]*pipeline-supervisor\/env/,
    "Tugboat must not source the whole supervisor env file",
  );
});
...
test("tugboat attaches --events-file watch from live loop_run_handoff while train runs (#1184)", () => {
  ...
  try {
    ...
    const src = fs.readFileSync(tugboat, "utf8");
    const helpers = [
      extractNamedFn(src, "read_supervisor_env_key", "tugboat.sh"),
      extractNamedFn(src, "present_unset_buzz_vars", "tugboat.sh"),
      extractNamedFn(src, "warn_if_buzz_credentials_missing", "tugboat.sh"),
      extractNamedFn(src, "extract_loop_run_handoff_events", "tugboat.sh"),
      extractNamedFn(src, "extract_latest_loop_run_handoff_events", "tugboat.sh"),
      ...
      extractNamedFn(src, "reap_stage_watch_pid", "tugboat.sh"),
...
test("tugboat re-binds stage-watch to a later distinct handoff while train is live (#1227)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-watch-rebind-"));
  try {
    const eventsA = path.join(dir, "run-a", "events.jsonl");
    const eventsB = path.join(dir, "run-b", "events.jsonl");
    fs.mkdirSync(path.dirname(eventsA), { recursive: true });
    fs.mkdirSync(path.dirname(eventsB), { recursive: true });
    fs.writeFileSync(eventsA, "");
    fs.writeFileSync(eventsB, "");
    const stderrFile = path.join(dir, "train.stderr");
    const argvFile = path.join(dir, "watch.argv");
    const logFile = path.join(dir, "playbook.log");
    const pidFile = path.join(dir, "stage-watch.pid");
    ...
    const handoff = (events: string, runId: string) =>
      JSON.stringify({
        schema_version: "1",
        kind: "loop_run_handoff",
        run_id: runId,
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "(",
        "  sleep 0.2",
        `  printf '%s\\n' ${JSON.stringify(handoff(eventsA, "loop-a"))}`,
        "  sleep 0.6",
        `  printf '%s\\n' ${JSON.stringify(handoff(eventsB, "loop-b"))}`,
        "  sleep 0.8",
        `) >>"$stderr" &`,
        "train_pid=$!",
        `attach_train_stage_watch "$stderr" "$train_pid" "1.40.0"`,
        "echo ATTACHED",
        'wait "$train_pid"',
        "echo TRAIN_DONE",
    ...
    const eventsArgs: string[] = [];
    const lines = argv.split("\n");
    for (let i = 0; i < lines.length; i++) {
      if (lines[i] === "--events-file" && lines[i + 1]) eventsArgs.push(lines[i + 1]);
    }
    assert.ok(eventsArgs.includes(eventsA), `first handoff missing: ${argv}`);
    assert.ok(eventsArgs.includes(eventsB), `second handoff not re-bound: ${argv}`);
    assert.ok(!argv.includes("--milestone"), `watch argv must not pass --milestone: ${argv}`);
    assert.doesNotMatch(argv, /AGENT_PIPELINE_LOOP_ROOT|\.local\/state\/agent-pipeline/);
  } finally {
...
test("tugboat does not respawn the same events path after watch exit (#1227)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-watch-no-respawn-"));
  try {
    const events = path.join(dir, "run", "events.jsonl");
    fs.mkdirSync(path.dirname(events), { recursive: true });
    ...
    assert.equal(r.status, 0, `fixture exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    const argv = fs.existsSync(argvFile) ? fs.readFileSync(argvFile, "utf8") : "";
    const spawnCount = (argv.match(/^SPAWN$/gm) ?? []).length;
    assert.equal(spawnCount, 1, `same path must not be respawned; argv=${argv}`);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
    fs.rmSync(dir, { recursive: true, force: true });
  }
...
test("tugboat ship-start reap kills leftover stage-watch pid-file process (#1227)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-watch-reap-"));
  const otherDir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-watch-other-"));
  try {
    const pidFile = path.join(dir, "stage-watch.pid");
    const otherPidFile = path.join(otherDir, "stage-watch.pid");
    const src = fs.readFileSync(tugboat, "utf8");
    const reap = extractNamedFn(src, "reap_stage_watch_pid", "tugboat.sh");
    const runner = path.join(dir, "run.sh");
    fs.writeFileSync(
      runner,
      [
        ...
        "sleep 120 &",
        "leftover=$!",
        'printf "%s\\n" "$leftover" >"$STAGE_WATCH_PID_FILE"',
        "sleep 120 &",
        "other=$!",
        `printf '%s\\n' "$other" > ${JSON.stringify(otherPidFile)}`,
        "reap_stage_watch_pid",
        "echo REAPED",
        'if kill -0 "$leftover" 2>/dev/null; then echo LEFTOVER_LIVE; else echo LEFTOVER_DEAD; fi',
        'if kill -0 "$other" 2>/dev/null; then echo OTHER_LIVE; else echo OTHER_DEAD; fi',
        'kill "$other" 2>/dev/null || true',
        "",
      ].join("\n"),
    );
    fs.chmodSync(runner, 0o755);
    const r = spawnSync("bash", [runner], { encoding: "utf8", timeout: 5000 });
    assert.equal(r.status, 0, `reap exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    assert.match(r.stdout, /REAPED/);
    assert.match(r.stdout, /LEFTOVER_DEAD/);
    assert.match(r.stdout, /OTHER_LIVE/);
    assert.equal(fs.existsSync(pidFile), false, "leftover pid-file must be removed");
    assert.ok(fs.existsSync(otherPidFile), "other-milestone pid-file must remain");
    assert.doesNotMatch(reap, /pkill/);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
function withoutHostBuzzEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const next = { ...env };
  delete next.BUZZ_BIN;
  delete next.BUZZ_CHANNEL;
  delete next.BUZZ_CREDENTIALS_FILE;
  delete next.BUZZ_RELAY_URL;
  delete next.XDG_CONFIG_HOME;
  return next;
}

function writeFakeWatchEnvDump(
  fakeWatch: string,
  ...
  const lines = [
    "#!/usr/bin/env bash",
    "set -euo pipefail",
    ...Object.entries(dumps).map(
      ([envName, file]) =>
        `printf '%s\\n' "\${${envName}-}" > ${JSON.stringify(file)}`,
    ),
    'pid_file=""',
    "while [[ $# -gt 0 ]]; do",
    '  case "$1" in',
  ...
  fs.writeFileSync(fakeWatch, lines.join("\n"), { mode: 0o755 });
}
...
test("tugboat watch spawn env includes parent BUZZ_CREDENTIALS_FILE (#1221)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-buzz-parent-"));
  try {
    ...
    writeStubFilter(filterPath);
    const creds = path.join(dir, "parent-creds.json");
    ...
    const envRelayFile = path.join(dir, "spawn.relay");
    const envChannelFile = path.join(dir, "spawn.channel");
    const fakeWatch = path.join(dir, "fake-watch");
    writeFakeWatchEnvDump(fakeWatch, {
      BUZZ_CREDENTIALS_FILE: envCredsFile,
      BUZZ_RELAY_URL: envRelayFile,
      BUZZ_CHANNEL: envChannelFile,
    });
    const src = fs.readFileSync(tugboat, "utf8");
    const runner = path.join(dir, "run.sh");
    fs.writeFileSync(
      ...
      [
        ...
        `HOME=${JSON.stringify(home)}`,
        `CODEX_HOME=${JSON.stringify(codexHome)}`,
        `BUZZ_CREDENTIALS_FILE=${JSON.stringify(creds)}`,
        'BUZZ_RELAY_URL="wss://example.invalid"',
        'BUZZ_CHANNEL="ch-parent"',
        "export HOME CODEX_HOME BUZZ_CREDENTIALS_FILE BUZZ_RELAY_URL BUZZ_CHANNEL",
        `LOG_FILE=${JSON.stringify(logFile)}`,
        "log() { echo \"$*\" | tee -a \"$LOG_FILE\"; }",
        `RUN_DIR=${JSON.stringify(dir)}`,
        `STAGE_WATCH_PID_FILE=${JSON.stringify(pidFile)}`,
    ...
    fs.chmodSync(runner, 0o755);
    runnerEnv.BUZZ_CREDENTIALS_FILE = creds;
    runnerEnv.BUZZ_RELAY_URL = "wss://example.invalid";
    runnerEnv.BUZZ_CHANNEL = "ch-parent";
    const r = spawnSync("bash", [runner], {
      encoding: "utf8",
      timeout: 10000,
      env: runnerEnv,
    ...
    assert.equal(fs.readFileSync(envRelayFile, "utf8").trim(), "wss://example.invalid");
    assert.equal(fs.readFileSync(envChannelFile, "utf8").trim(), "ch-parent");
    assert.match(r.stdout, /TRAIN_CONTINUED/);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
    fs.rmSync(dir, { recursive: true, force: true });
...
test("tugboat fills unset Buzz vars from supervisor env file without sourcing (#1221)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-buzz-envfile-"));
  try {
    const { home, codexHome, runnerEnv: baseEnv } = isolatedFilterHomes(dir);
    const runnerEnv = withoutHostBuzzEnv(baseEnv);
    const filterPath = path.join(
    ...
    writeStubFilter(filterPath);
    const creds = path.join(dir, "env-creds.json");
    fs.writeFileSync(creds, JSON.stringify({ nsec: "nsec1fixture-not-real" }));
    const envDir = path.join(home, ".config", "pipeline-supervisor");
    fs.mkdirSync(envDir, { recursive: true });
    const pinnedRepo = path.join(dir, "pinned-repo");
    fs.mkdirSync(pinnedRepo, { recursive: true });
    fs.writeFileSync(
      path.join(envDir, "env"),
      [
        `REPO_DIR=/should-not-apply-from-env-file`,
        `BUZZ_CREDENTIALS_FILE=${creds}`,
        "BUZZ_RELAY_URL=wss://from-env-file.invalid",
        "BUZZ_CHANNEL=ch-from-env-file",
        "",
      ].join("\n"),
    );
    const events = path.join(dir, "events.jsonl");
    ...
    fs.writeFileSync(
      ...
      [
        ...
        `SHIP_STAGE_WATCH_BIN=${JSON.stringify(fakeWatch)}`,
        `STATE_ROOT=${JSON.stringify(dir)}`,
        `REPO_DIR=${JSON.stringify(pinnedRepo)}`,
        "SHIP_NOTIFY=1",
        "SHIP_NOTIFY_BIN=/bin/true",
        extractTrainWatchHelpers(src),
        `start_train_stage_watch ${JSON.stringify(events)} "1.40.0"`,
        'printf "REPO_DIR_AFTER=%s\\n" "$REPO_DIR"',
        "echo TRAIN_CONTINUED",
        "",
      ].join("\n"),
    );
    ...
    assert.equal(r.status, 0, `fixture exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    assert.equal(fs.readFileSync(envCredsFile, "utf8").trim(), creds);
    assert.equal(fs.readFileSync(envRelayFile, "utf8").trim(), "wss://from-env-file.invalid");
    assert.equal(fs.readFileSync(envChannelFile, "utf8").trim(), "ch-from-env-file");
    assert.match(r.stdout, new RegExp(`REPO_DIR_AFTER=${pinnedRepo.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`));
    assert.doesNotMatch(r.stdout, /should-not-apply-from-env-file/);
    assert.match(r.stdout, /TRAIN_CONTINUED/);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
    fs.rmSync(dir, { recursive: true, force: true });
...
test("tugboat preserves operator-set BUZZ_CREDENTIALS_FILE over supervisor env file (#1221)", () => {
  ...
  try {
    ...
    writeStubFilter(filterPath);
    const operatorCreds = path.join(dir, "operator-creds.json");
    const fileCreds = path.join(dir, "file-creds.json");
    fs.writeFileSync(operatorCreds, JSON.stringify({ nsec: "nsec1operator" }));
    fs.writeFileSync(fileCreds, JSON.stringify({ nsec: "nsec1file" }));
    const envDir = path.join(home, ".config", "pipeline-supervisor");
    fs.mkdirSync(envDir, { recursive: true });
    fs.writeFileSync(
      path.join(envDir, "env"),
      `BUZZ_CREDENTIALS_FILE=${fileCreds}\n`,
    );
    const events = path.join(dir, "events.jsonl");
    fs.writeFileSync(events, "");
    const logFile = path.join(dir, "playbook.log");
    ...
    fs.writeFileSync(
      ...
      [
        ...
        `HOME=${JSON.stringify(home)}`,
        `CODEX_HOME=${JSON.stringify(codexHome)}`,
        `BUZZ_CREDENTIALS_FILE=${JSON.stringify(operatorCreds)}`,
        "export HOME CODEX_HOME BUZZ_CREDENTIALS_FILE",
        `LOG_FILE=${JSON.stringify(logFile)}`,
        "log() { echo \"$*\" | tee -a \"$LOG_FILE\"; }",
        `RUN_DIR=${JSON.stringify(dir)}`,
        `STAGE_WATCH_PID_FILE=${JSON.stringify(pidFile)}`,
    ...
    fs.chmodSync(runner, 0o755);
    runnerEnv.BUZZ_CREDENTIALS_FILE = operatorCreds;
    const r = spawnSync("bash", [runner], {
      encoding: "utf8",
      timeout: 10000,
      env: runnerEnv,
    ...
    killPidFile(pidFile);
    assert.equal(r.status, 0, `fixture exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    assert.equal(fs.readFileSync(envCredsFile, "utf8").trim(), operatorCreds);
    assert.notEqual(fs.readFileSync(envCredsFile, "utf8").trim(), fileCreds);
    assert.match(r.stdout, /TRAIN_CONTINUED/);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
    fs.rmSync(dir, { recursive: true, force: true });
...
test("tugboat logs buzz credentials missing and does not fail train (#1221)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-buzz-missing-"));
  try {
    const { home, codexHome, runnerEnv: baseEnv } = isolatedFilterHomes(dir);
    const runnerEnv = withoutHostBuzzEnv(baseEnv);
    const filterPath = path.join(
    ...
    writeStubFilter(filterPath);
    const buzzBin = path.join(dir, "fake-buzz");
    fs.writeFileSync(buzzBin, "#!/usr/bin/env bash\nexit 0\n", { mode: 0o755 });
    const events = path.join(dir, "events.jsonl");
    fs.writeFileSync(events, "");
    const logFile = path.join(dir, "playbook.log");
    const pidFile = path.join(dir, "stage-watch.pid");
    ...
    fs.writeFileSync(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "unset BUZZ_CREDENTIALS_FILE XDG_CONFIG_HOME",
        `HOME=${JSON.stringify(home)}`,
        `CODEX_HOME=${JSON.stringify(codexHome)}`,
        `BUZZ_BIN=${JSON.stringify(buzzBin)}`,
        'BUZZ_CHANNEL="ch-fixture"',
        "SHIP_NOTIFY=1",
        "export HOME CODEX_HOME BUZZ_BIN BUZZ_CHANNEL SHIP_NOTIFY",
        `LOG_FILE=${JSON.stringify(logFile)}`,
        "log() { echo \"$*\" | tee -a \"$LOG_FILE\"; }",
        `RUN_DIR=${JSON.stringify(dir)}`,
        `STAGE_WATCH_PID_FILE=${JSON.stringify(pidFile)}`,
    ...
    fs.chmodSync(runner, 0o755);
    runnerEnv.BUZZ_BIN = buzzBin;
    runnerEnv.BUZZ_CHANNEL = "ch-fixture";
    runnerEnv.SHIP_NOTIFY = "1";
    const r = spawnSync("bash", [runner], {
      encoding: "utf8",
      timeout: 10000,
      env: runnerEnv,
    ...
    assert.equal(r.status, 0, `fixture exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    const combined = `${r.stdout}\n${fs.existsSync(logFile) ? fs.readFileSync(logFile, "utf8") : ""}`;
    assert.match(combined, /buzz credentials missing/);
    assert.match(r.stdout, /TRAIN_CONTINUED/);
    assert.doesNotMatch(combined, /stage-watch argv rejected/);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
...
test("tugboat does not log buzz credentials missing when BUZZ_BIN is empty (#1221)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-buzz-emptybin-"));
  try {
    const { home, codexHome, runnerEnv: baseEnv } = isolatedFilterHomes(dir);
    const runnerEnv = withoutHostBuzzEnv(baseEnv);
    const filterPath = path.join(
    ...
    fs.writeFileSync(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "unset BUZZ_BIN BUZZ_CREDENTIALS_FILE XDG_CONFIG_HOME",
        `HOME=${JSON.stringify(home)}`,
        `CODEX_HOME=${JSON.stringify(codexHome)}`,
        "SHIP_NOTIFY=1",
        "export HOME CODEX_HOME SHIP_NOTIFY",
        `LOG_FILE=${JSON.stringify(logFile)}`,
        "log() { echo \"$*\" | tee -a \"$LOG_FILE\"; }",
        `RUN_DIR=${JSON.stringify(dir)}`,
        `STAGE_WATCH_PID_FILE=${JSON.stringify(pidFile)}`,
    ...
    assert.equal(r.status, 0, `fixture exited ${r.status}: ${r.stdout}\n${r.stderr}`);
    const combined = `${r.stdout}\n${fs.existsSync(logFile) ? fs.readFileSync(logFile, "utf8") : ""}`;
    assert.doesNotMatch(combined, /buzz credentials missing/);
    assert.match(r.stdout, /TRAIN_CONTINUED/);
  } finally {
    killPidFile(path.join(dir, "stage-watch.pid"));
    fs.rmSync(dir, { recursive: true, force: true });
  ...
  try {
    const r = spawnSync("bash", [tugboat, "--milestone", "v1.39.0", "--skip-frg"], {
      ...
      env: tugboatSpawnEnv(
        ...
        { parentEnv: tugboatReleaseCiParentEnv() },
      ),
    });
...
test("tugboatSpawnEnv omits inherited skip-train unless asserting skip-train (#1192)", () => {
  ...
  assert.equal(isolated.TUGBOAT_REPOSITORY, "accidental-hedge-fund/agent-pipeline");
  const inherited = tugboatSpawnEnv(
    {},
...
test("named #1150 / #1151 spawn-real tugboat tests keep original FRG and candidate assertions (#1192)", () => {
  ...
  assert.match(src, /tugboat after train-complete records candidate argv not pin argv \(#1151\)/);
  assert.match(src, /tugboat live in_progress at cap 1 keeps ticking prepare \(#1150\)/);
  assert.match(src, /tugboat not-live in_progress at cap 1 fails closed \(#1150\)/);
  assert.match(src, /tugboat fails closed when candidate engine is unavailable \(#1151\)/);
  assert.match(src, /assert\.match\(candArgv, \/factory-release\/\)/);
  assert.match(src, /assert\.match\(pinArgv, \/train\/\)/);
  assert.match(src, /assert\.equal\(ticks, 2/);
...
test("parent skip-train env without isolation fails spawn-real tugboat fixtures (#1192)", () => {
  const inherit: TugboatSpawnEnvOpts = {
    inheritParentSkipTrain: true,
    parentEnv: tugboatReleaseCiParentEnv(),
  };
  const skipTrainFail = /TUGBOAT_SKIP_TRAIN without train.complete.json or train.json/;

  const argvDir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-skip-bite-argv-"));
  try {
    const fx = writeTugboatFrgFixture(argvDir, {
      launcherBody: candidateArgvLauncher(path.join(argvDir, "cand-record")),
    });
    const r = spawnTugboatMilestone(tugboatFrgSpawnEnv(argvDir, fx, {}, inherit));
    const combined = `${r.stdout}\n${r.stderr}`;
    assert.notEqual(r.status, 0);
    assert.match(combined, skipTrainFail);
    const candArgv = fs.existsSync(path.join(fx.candRecord, "argv"))
      ? fs.readFileSync(path.join(fx.candRecord, "argv"), "utf8")
      : "";
    const pinArgv = fs.existsSync(path.join(fx.pinRecord, "argv"))
      ? fs.readFileSync(path.join(fx.pinRecord, "argv"), "utf8")
      : "";
    assert.doesNotMatch(candArgv, /factory-release/);
    assert.doesNotMatch(pinArgv, /train/);
    assert.equal(skipTrainProofKind(path.join(fx.state, "ship-v1.39.5")), null);
  } finally {
    fs.rmSync(argvDir, { recursive: true, force: true });
  }

  const liveDir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-skip-bite-live-"));
  try {
    const ticksPath = path.join(liveDir, "prepare-ticks");
    const fx = writeTugboatFrgFixture(liveDir, {
      launcherBody: liveWaitLauncher(path.join(liveDir, "cand-record"), ticksPath),
    });
    const r = spawnTugboatMilestone(
      tugboatFrgSpawnEnv(
        liveDir,
        fx,
        { AGENT_PIPELINE_STATE_HOME: path.join(liveDir, "loop-home") },
        inherit,
      ),
    );
    assert.notEqual(r.status, 0);
    assert.match(`${r.stdout}\n${r.stderr}`, skipTrainFail);
    assert.equal(fs.existsSync(ticksPath), false);
  } finally {
    fs.rmSync(liveDir, { recursive: true, force: true });
  }

  const deadDir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-skip-bite-dead-"));
  try {
    const ticksPath = path.join(deadDir, "prepare-ticks");
    const fx = writeTugboatFrgFixture(deadDir, {
      launcherBody: notLiveWaitLauncher(path.join(deadDir, "cand-record"), ticksPath),
    });
    const r = spawnTugboatMilestone(
      tugboatFrgSpawnEnv(
        deadDir,
        fx,
        { AGENT_PIPELINE_STATE_HOME: path.join(deadDir, "empty-loop-home") },
        inherit,
      ),
    );
    const combined = `${r.stdout}\n${r.stderr}`;
    assert.notEqual(r.status, 0);
    assert.match(combined, skipTrainFail);
    assert.doesNotMatch(combined, /FRG pack still in_progress within wait budget/);
  } finally {
    fs.rmSync(deadDir, { recursive: true, force: true });
  }

  const noCandDir = fs.mkdtempSync(path.join(os.tmpdir(), "tugboat-skip-bite-nocand-"));
  try {
    const fx = writeUnavailableEngineFixture(noCandDir);
    const r = spawnTugboatMilestone(
      tugboatUnavailableEngineSpawnEnv(noCandDir, fx.pin, fx.repo, inherit),
    );
    const combined = `${r.stdout}\n${r.stderr}`;
    assert.notEqual(r.status, 0);
    assert.match(combined, skipTrainFail);
    assert.doesNotMatch(combined, /candidate-engine identity defect|cannot resolve candidate engine/);
    const pinArgv = fs.existsSync(path.join(fx.pinRecord, "argv"))
      ? fs.readFileSync(path.join(fx.pinRecord, "argv"), "utf8")
      : "";
    ...
    fs.rmSync(noCandDir, { recursive: true, force: true });
  }
});
...
test("tugboat after train-complete records candidate argv not pin argv (#1151)", () => {
  ...
  try {
    ...
    const fx = writeTugboatFrgFixture(dir, {
      launcherBody: candidateArgvLauncher(candRecord),
    });
    const r = spawnTugboatMilestone(
      tugboatFrgSpawnEnv(dir, fx, {}, { parentEnv: tugboatReleaseCiParentEnv() }),
    );
    const candArgv = fs.existsSync(path.join(fx.candRecord, "argv"))
      ? fs.readFileSync(path.join(fx.candRecord, "argv"), "utf8")
      : "";
    ...
    assert.doesNotMatch(pinArgv, /factory-release/);
    const candEnv = fs.existsSync(path.join(fx.candRecord, "env"))
      ? fs.readFileSync(path.join(fx.candRecord, "env"), "utf8")
      : "";
    ...
    assertSkipTrainProof(path.join(fx.state, "ship-v1.39.5"));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
function writeTugboatFrgFixture(dir: string, opts: {
  ...
  fs.writeFileSync(
    ...
    fs.readFileSync(pin, "utf8") +
      [
        'if [[ "$1" == "train" ]]; then',
        // Fake complete:true so ensure_train_complete_artifact leaves skip-train proof (#1192).
...
function tugboatFrgSpawnEnv(
  dir: string,
  fx: ReturnType<typeof writeTugboatFrgFixture>,
  extra: NodeJS.ProcessEnv = {},
  opts: TugboatSpawnEnvOpts = {},
): NodeJS.ProcessEnv {
  return tugboatSpawnEnv(
    {
...
function candidateArgvLauncher(candRecord: string): string[] {
  return [
...
function liveWaitLauncher(candRecord: string, ticksPath: string): string[] {
  return [
...
function notLiveWaitLauncher(candRecord: string, ticksPath: string): string[] {
  return [
    "import { appendFileSync, readFileSync, writeFileSync } from 'node:fs';",
    `const rec = ${JSON.stringify(candRecord)};`,
    `const ticksPath = ${JSON.stringify(ticksPath)};`,
...
function writeUnavailableEngineFixture(dir: string): {
  pin: string;
  pinRecord: string;
  repo: string;
} {
  ...
  return { pin, pinRecord, repo };
}

function tugboatUnavailableEngineSpawnEnv(
  dir: string,
  pin: string,
  repo: string,
  opts: TugboatSpawnEnvOpts = {},
  ...
  return tugboatSpawnEnv(
    {
      PATH: `${path.join(dir, "pin-bin")}${path.delimiter}${process.env.PATH ?? ""}`,
      PIPELINE: pin,
      REPO_DIR: repo,
      ALLOW_MERGE: "1",
      SHIP_NOTIFY: "0",
...
function spawnTugboatMilestone(
  env: NodeJS.ProcessEnv,
  version = "v1.39.5",
): ReturnType<typeof spawnSync> {
  return spawnSync("bash", [tugboat, "--milestone", version], {
    encoding: "utf8",
    env,
  });
}
...
test("tugboat live in_progress at cap 1 keeps ticking prepare (#1150)", () => {
  ...
  try {
    ...
    const fx = writeTugboatFrgFixture(dir, {
      launcherBody: liveWaitLauncher(path.join(dir, "cand-record"), ticksPath),
    });
    const r = spawnTugboatMilestone(
      tugboatFrgSpawnEnv(
        dir,
...
test("tugboat not-live in_progress at cap 1 fails closed (#1150)", () => {
  ...
  try {
    const ticksPath = path.join(dir, "prepare-ticks");
    const fx = writeTugboatFrgFixture(dir, {
      launcherBody: notLiveWaitLauncher(path.join(dir, "cand-record"), ticksPath),
    });
    const r = spawnTugboatMilestone(
      tugboatFrgSpawnEnv(
        dir,
...
test("tugboat fails closed when candidate engine is unavailable (#1151)", () => {
  ...
  try {
    const fx = writeUnavailableEngineFixture(dir);
    const r = spawnTugboatMilestone(
      tugboatUnavailableEngineSpawnEnv(dir, fx.pin, fx.repo, {
        parentEnv: tugboatReleaseCiParentEnv(),
      }),
    );
    assert.notEqual(r.status, 0);
    ...
    assert.doesNotMatch(pinArgv, /factory-release/);
    assert.match(pinArgv, /train/);
    assertSkipTrainProof(path.join(dir, "state", "ship-v1.39.5"));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test("tugboat after train re-execs candidate tugboat.sh and skip-train omits train (#1164)", () => {
  ...
  assert.match(reexec, /export PIPELINE_SUPERVISOR_STATE="\$STATE_ROOT"/);
  assert.match(reexec, /export REPO_DIR/);
  ...
  try {
    ...
    const stateRoot = path.join(dir, "state");
    const repoDir = path.join(dir, "control");
    const runDir = path.join(stateRoot, "ship-v1.39.6");
    fs.mkdirSync(runDir, { recursive: true });
    fs.mkdirSync(repoDir, { recursive: true });
    fs.writeFileSync(
      path.join(runDir, "train.complete.json"),
      '{"schema_version":1,"kind":"train_status","complete":true}\n',
    );
    fs.writeFileSync(
      ...
      [
        ...
        "run_dir=\"$state_root/ship-v1.39.6\"",
        "proof=PROOF_MISS",
        "if [[ -s \"$run_dir/train.complete.json\" ]]; then proof=PROOF_OK; fi",
        "printf '%s\\n' \"reexec skip=${TUGBOAT_SKIP_TRAIN:-}\" > " + JSON.stringify(marker),
        "printf '%s\\n' \"state=${PIPELINE_SUPERVISOR_STATE:-}\" >> " + JSON.stringify(marker),
        "printf '%s\\n' \"repo=${REPO_DIR:-}\" >> " + JSON.stringify(marker),
        "printf '%s\\n' \"$proof\" >> " + JSON.stringify(marker),
        "exit 0",
        "",
      ].join("\n"),
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        // Shell locals, not exported: re-exec must export them (#1192).
        `STATE_ROOT=${JSON.stringify(stateRoot)}`,
        `REPO_DIR=${JSON.stringify(repoDir)}`,
        "unset PIPELINE_SUPERVISOR_STATE || true",
        extractNamedFn(src, "is_exact_git_sha", "tugboat.sh"),
    ...
    assert.match(markerText, new RegExp(`^state=${stateRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "m"));
    assert.match(markerText, new RegExp(`^repo=${repoDir.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "m"));
    assert.match(markerText, /^PROOF_OK$/m);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
