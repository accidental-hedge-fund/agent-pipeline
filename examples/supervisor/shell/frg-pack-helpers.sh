...
# Bound pack loop liveness from candidate prepare JSON field `liveness.status`.
# Prints 1 (live), 0 (not-live), unknown, or failed. $1 = prepare JSON path.
# Does not inspect the ledger. `--engine-track candidate` is not a binary selector.
...
# Wait-continue vs wait-fail. $1 tick (retry|done|attest|fail) $2 live (1|0|unknown|failed)
# $3 attempt (1-based) $4 numeric cap. $5 optional attest_allowance_spent (1|0).
# retry + live or unknown continues even at cap.
# failed identity is fail-closed and does not continue-as-live.
