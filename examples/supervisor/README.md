...
| `ship-stage-watch.sh` | Stream one explicit run event file through `material-filter.mjs` (shared). Follow exits on that file's identity-terminal; Tugboat re-binds `--events-file` from a later train handoff. |
